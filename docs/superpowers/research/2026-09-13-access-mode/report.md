...
The earlier broad Apple review remains platform-blocked and incomplete. This new policy review does not resolve that block or the original whole-PR gates. The PR remains draft; no merge, deployment, phone install or real provider login occurred. James's reported Apple host setup is preserved, while the new `ACCESS_MODE=restricted` host edit is still unconfirmed.
