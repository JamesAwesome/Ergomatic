...
**Architecture:** Two small adapter-layer additions following the house native-first idiom (`isNative()` picks the arm; the native arm reaches its Capacitor plugin only through a dynamic `import()` so the plugin never lands in the web bundle — `adapters/appLifecycle.ts` / `keepAwake.ts` are the precedents). Everything ships dark: nothing renders until PR2, and the server refuses while `C2_LINK_ENABLED` is off.
...
**Spec:** `docs/superpowers/specs/2026-08-31-concept2-logbook-design.md` §Architecture 2-3 + §PR decomposition (PR1.5). The spec's gate plan: PR1.5's mechanics are anchor-vetted ground; the antagonist SKIP is spoken here — "inherits the phase anchor; no new invariant class in the plumbing" — EXCEPT the design-gate evidence (fresh ground, reviewed by James at the gate itself).
...
- [ ] `pnpm lint && pnpm format:check && pnpm typecheck && pnpm test` (all projects; Docker for integration), per-file coverage on the two new src files (RF2), `pnpm build && pnpm dist:grep` (7 needles still green; `@capacitor/browser` must fold out of the web bundle — grep `dist/client` for `SFSafariViewController`/`@capacitor/browser` both directions with a temporary static-import plant for the red proof, RF12).
- [ ] **`pnpm e2e`** (RF1 — src touched). Stack teardown afterwards is the phase teardown's job (`docker compose -p <name> down -v`).
- [ ] PR body: house shape, ~120 words, Record block carries the mutation ledger, the spoken antagonist skip, the plan corrections, and the gate package pointer. James reviews; the design-gate ruling is requested IN the PR presentation. No merge without his word.
