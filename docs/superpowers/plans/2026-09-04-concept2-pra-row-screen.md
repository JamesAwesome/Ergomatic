...
- **R5 — the screen always answers.** Chrome (BackLink + title) in every state. Redirect predicate is EXACTLY `link !== null && !link.available`. Never `!link?.available`, never `link === null` (AUD-015). It never renders nothing while its own read is the authority — the card's second hook can go silent underneath it (the accepted disagreement window, `Concept2Screen.test.tsx`'s disagreement case; Task 8's Record entry "The known window").
