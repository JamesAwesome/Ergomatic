...
label, both RULED KEEP. **That is not the same as nothing being owed him**, and
RC-38 below is one of them. The Phase OD sweep found seven live decisions
waiting on James, each findable by NAME rather than by a line a merge would
break — the hand-verified concept2.com log-dev row, the belted 0x0039 capture
owed on the next walk, the Wave E / Phase LP flag-flip trip, the
`DETAIL`-over-slug failure panel, and three scheduled to be RE-ASKED at Wave A's
close. **That is SIX now, not seven: the third re-ask was Phase PROTO's sweep,
and James removed that phase on 2026-09-10** (`docs/history/phase-proto.md`).
The two that stand are the plan calendar and the parametric generator. A reader
who takes this section's near-emptiness as "he owes nothing" will be wrong by
six.

- **RC-38, lifted from Phase PROTO 2026-09-10 — BLOCKED ON JAMES.**
...
| **RC-38**                                  | Transcribe `OBJ_WORKOUTTYPE_T`. Pulled forward alone by James on 2026-08-31 while the rest of the sweep was held; the sweep (Phase PROTO) was REMOVED 2026-09-10 and RC-38's full row moved to `## Needs a decision from James` above — it is blocked on a document only he can supply                                                                                                                                                                                                                                                                                                                       | `phase-rc.md`                |
