...
4. **Still open after day 2:** the confirming row for finding 3's fix (a completed piece reading "ALL 1 INTERVALS MEASURED", with a stash containing `split-half` x2 → `record-actual accepted` → `handoff-released final-boundary`), and one unexplained observation — `localStorage["ergomatic.monitorRun"]` read as `null` on the save screen before saving, which no code path on the ended→log route explains (the record's own loader deletes a record that fails its shape check, which is the first thing to rule out if it recurs).
