...
- [ ] **Step 1: Verify versions and install**
...
- [ ] **Step 2: Move `server/db.ts` → `server/db/pool.ts`** (contents unchanged — createPool with the error listener, checkDb) and fix the three importing test/files plus `index.ts` (`./db.js` → `./db/pool.js`). Run `pnpm typecheck && pnpm test` — everything still green before any new code.

- [ ] **Step 3: Write `app/server/db/schema.ts`**
...
- [ ] **Step 4: Write `app/server/db/index.ts` and `app/drizzle.config.ts`**
...
- [ ] **Step 6: Write the failing integration test `app/server/db/schema.integration.test.ts`**
...
- [ ] **Step 7: Wire migrate-on-boot in `app/server/index.ts`**
...
- [ ] **Step 8: Dockerfile — copy migrations into the runtime image** (after the dist COPY):
...
- [ ] **Step 2: Failing unit tests — `app/server/auth/allowlist.test.ts`**
...
- [ ] **Step 5: GREEN on unit; then the failing integration test `app/server/auth/sessions.integration.test.ts`**
...
- [ ] **Step 6: Run both projects; commit**
...
- [ ] **Step 1: Failing tests `app/server/auth/middleware.test.ts`** (mini express apps + supertest; a fake store implementing just `resolveSession`)
...
- [ ] **Step 2: RED** — `pnpm test --project unit` fails on missing `./middleware.js`.

- [ ] **Step 3: Implement `app/server/auth/middleware.ts`**
...
- [ ] **Step 2: Write `app/server/auth/users.ts`**
...
- [ ] **Step 3: Write `app/server/auth/google.ts`** (thin wrapper; VERIFY names against the installed openid-client v6 README if typecheck disagrees — expected API as of 6.8.4 shown)
...
- [ ] **Step 4: Failing route tests `app/server/auth/routes.test.ts`** (stubbed provider = branch testing; the exchange itself is deliberately untested per spec)
...
- [ ] **Step 5: RED**, then implement `app/server/auth/routes.ts`
...
- [ ] **Step 6: Rewire `app/server/app.ts`**
...
- [ ] **Step 7: Update the three existing test files** — add a shared helper `app/server/testDeps.ts`:
...
- [ ] **Step 8: Wire real deps in `app/server/index.ts`** (after the migrate call from Task 1)
...
- [ ] **Step 9: GREEN everything + commit**
...
- [ ] **Step 1: Failing tests.** `app/src/App.test.tsx` becomes:
...
- [ ] **Step 2: RED** — `pnpm test --project client` fails.

- [ ] **Step 3: Implement.** `app/src/useMe.ts`:
...
- [ ] **Step 1: compose.yml** — add to the `app` service `environment` block:
...
- [ ] **Step 2: .env.example** — after the SITE_URL line:
...
- [ ] **Step 3: docs/deploy.md** — append a `## Google sign-in (one-time)` section:
...
- [ ] **Step 4: CLAUDE.md** — under Commands, after the dev-db line:
...
- [ ] **Step 5: Full local verification** (compose stack end-to-end, auth disabled — proves the no-Google path deploys clean):
...
- [ ] **Step 1 [JAMES]:** docs/deploy.md "Google sign-in" steps 1–5 (Cloud Console client, host `.env` values incl. `ALLOWED_EMAILS` with both testers' emails, container recreate).
- [ ] **Step 2:** Merge the PR (rebase) → deploy runs → `gh run watch --exit-status` green. Confirm boot log shows `migrations up to date` and NO Google warnings: the env is live.
- [ ] **Step 3 [JAMES + controller]:** Live verification on https://ergomatic.waffle.haus:
...
- [ ] **Step 4:** Dependabot check (standing rule): `gh pr list` — if PR #5 (@types/node) is aged past pnpm's minimumReleaseAge (after 2026-07-28 ~17:30Z), re-run its CI and squash-merge.
- [ ] **Step 5:** Close out — ROADMAP Phase 2 → Done + boxes ticked, this plan's checkboxes ticked, ledger updated; commit via a small PR (main is PR-only).

---

## Exit criteria (ROADMAP Phase 2 / spec)

- [ ] Two allowlisted Google accounts hold isolated sessions on the live site (each sees their own /api/me)
- [ ] Non-allowlisted account politely refused; no user row created
- [ ] Deployed through CD; boot log clean (migrations + no auth warnings)
- [ ] Recorded obligation forward: Phase 3 adds the behavioral two-user DATA isolation test with its first data route
