...
- Targeted deadline and collision window are named, test-injected constants: `TARGET_SCAN_DEADLINE_MS = 10_000`, `TARGET_COLLISION_WINDOW_MS = 1_000`. Evidence: the first matching `requestLEScan` callback arrived 103 ms after scan start in one sample (`REPAIRED-NORMAL.md`, which says these are bridge callback timings, not over-the-air intervals) and 169 ms in another (`normal-trace-v8-receipt.json`, `firstMatchingAdvertisementMs`). The PM5's advertising interval is UNMEASURED (nothing in `docs/superpowers/research/` or the Concept2 interface material states it). The deadline is ~60× the worse first-match observation. **The collision window's job is the time until a second, DISTINCT device would first be seen, so its anchor is first-match latency, not duplicate spacing**: 1_000 ms is ~6× the worse observation, and it is paid on EVERY successful NFC connect as a full extra second after the match, before the interstitial moves on — a number James may shorten (a number change is his call; Task 11's walk records `✓ PM5 found` → READY). **The deadline bounds the whole targeted attempt, armed at entry** (lens 1, F5): every preflight call goes through BleClient's serial queue, which can sit behind a modal picker sheet; an expiry before the radio is live is `target-interrupted`, after it `target-not-advertising`. Cleanup (`stopLEScan()`) is bounded by the same constant and poisons on expiry (F6). Both are Ergomatic policy, per the spec's residual, and boundary tests use independent literals (`9_999`/`10_000`, `999`/`1_000`), never the constants.
...
- The checked-in NFC patch is part of the safety mechanism; its `nfcSessionEnd` carries `cause: "multipleTags" | "tagFailure"` with `reason: "invalidated"` on controller-forced endings (spec "Reader-ending seam"). `multipleTags` is minted for zero OR several delegate tags (the name is the spec's; the enum comment says so). `didDetectNDEFs:` is never called on this plugin (it implements `didDetectTags:`, which makes every session read-write — `NFCNDEFReaderSession.h`), so that rejection path and its Swift test are defence in depth only.
...
- Create `app/scripts/nul-check.sh` — the NUL-byte gate (F2), wired into `pnpm lint`.
...
Run it once BEFORE editing (save the output), and again after; paste both in the PR record. (Measured on the paste-test: every transport file had 0 `scanTarget` hits at `c5af7392`.)
...
Lifecycle: NO current-state read (lens 1, F1 — withdrawn; see Global Constraints). The bridge's `DecodedNfcEvent` has three rejections: `stale-id`, `invalid` (unattributable), `invalid-for-attempt` (this attempt's ID, malformed) — the third is terminal in both the native and the scripted reader.
...
**Interfaces:**
- `stageRetire(set, attemptId: ConnectionAttemptId)`; `takeStagedRetire(attemptId)` — KEYED too (lens 1, F7): returns the set only when the staged attempt ID equals the argument, otherwise discards it (receipted) and returns `[]`, so a set staged by a detail press whose NFC read ended quietly can never authorize JustRow's zero-argument `connect()`'s armed retire; `discardStagedRetire(attemptId: ConnectionAttemptId)` — discards ONLY if the staged attempt ID equals the argument (compare-by-ID; a late A cleanup cannot discard B's authorization); `stagedRetireAttemptId(): ConnectionAttemptId | null` (test accessor). Invariants (RF27): (a) a staged set authorizes exactly one attempt ID's armed retire and nothing else; (b) the hook's `attemptIdRef` is never cleared, and a stale value can only fail to match, never falsely match.
- Tasks 6 and 7 land in ONE commit: the keyed take needs the hook to know its attempt ID at `armed`, which is `connect(request)`.
...
- `ConnectedInterstitial` props gain `request: MonitorDiscoveryRequest`; mount effect and `handleTryAgain` call `session.connect(request)`; `canRetry` is false for `target-ambiguous` and `scan-cleanup-failed` (copy says so); for `target-already-connected` the button stays **Try again** and repeats the exact target. The mount-once effect CLAIMS the mount lease (`claimMountLease(request.attemptId)`, `onMountLeaseLost(request.attemptId, () => discardStagedRetire(request.attemptId))`, cleanup `release()`): a StrictMode replay reclaims within the microtask; a genuine unmount discards the still-staged receipt (a no-op after `armed` consumed it or Cancel discarded it).
- The hook validates the request BEFORE any radio call (`isValidAttemptId`, `isValidPm5AdvertisingName`) and fails closed as `transport-missing`; the scan's foreground lease is a `registerAppLifecycleListener` registered before `scanTarget` and removed in its `finally` — `background` aborts the controller.
...
Mapping (spec "Reader-ending seam" rule): records → parse → target or `Unsupported NFC tag`; `NfcCancelledError` → quiet; `NfcTimeoutError` → `No NFC tag detected. Try again.`; `NfcInvalidatedError` with `cause === "multipleTags"` → `Unsupported NFC tag`; any other `NfcInvalidatedError`, `NfcStartError` → `NFC scan stopped. Try again.`; `NfcAbortError` → quiet. The haptic is awaited with `.catch(() => trace.record("haptic-failed"))` and never delays: fire it, then `flushSync(() => onAccepted())` (lens 1, F10: the accepted state must be COMMITTED before the frames are counted — an async continuation's `setState` lands on a MessageChannel task whose order against the rendering steps is unspecified), then `await paint(signal)`; if `signal.aborted` after paint → quiet (keyed cleanup only). `paintBarrier`'s "two rAFs = one committed paint" is INFERENCE, stated as such in its comment; the barrier is abortable, never timed.
...
- state: `nfcCapability` (`useEffect` → `resolveNfcReader().capability()` with a 2_000 ms deadline — ~50× the measured `capabilityLatencyMs` of 41 ms in `normal-trace-v8-receipt.json`, a fail-closed bound; rejection/timeout keep `"unknown"` and trace `capability-failed`/`capability-timed-out`; **only `"supported"`/`"unsupported"` are cached** in a module-level variable for the process, never `"unknown"` (lens 1, F11: a timeout is not a resolution and must not hide the button for the whole process), never persisted), `entryBusy`, `nfcAccepted`, `connectError` (existing slot renders the three inline NFC strings too).
- `handleEntryProceed(intent)`: manual → today's `handleConnectProceed` body plus `request: { kind: "picker", attemptId }` into `connecting`; nfc → `entryBusy = true`, build trace, `runNfcAttempt(...)`, then on `target`: compile exactly as today (baselines guard, `buildNudgedDraft`, `buildRun`, `compileProgram`, `buildLogSeed`) and `setConnecting({ …, request: { kind: "advertised-name", attemptId, exactName: target.advertisingName } })`; on `inline-error`: `setConnectError(copy)`; on `quiet`: no user-visible change. **Invariant (lens 1, F7): every outcome except a `target` that reaches `setConnecting` discards by ID before returning to detail** — quiet, inline-error, baseline guard, compile failure. `entryBusy = false` and `nfcAccepted = false` on every path that stays on detail.
