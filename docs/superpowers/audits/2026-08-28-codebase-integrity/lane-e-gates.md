...
| Today / Library / History  | Seeded library, bulk-imported workouts, and API-created logs cover empty/populated History and row navigation (`app/e2e/log.spec.ts:173-245`)                                                               | Portrait default; selected scroll tests use 390×500 | Removing the row, hero, empty state, or detail link fails visible/URL assertions                                                | AUD-002 malformed 2xx, AUD-006 accepted leading/adjacent rest, AUD-011 storage getter denial, AUD-013 extreme JSONB; no landscape recovery matrix |
| Countdown / Timer / Log    | Bulk-imported workouts and real browser storage cover start, resume/BACK, completion, session/manual log doors (`app/e2e/session.spec.ts:183-390,654-1039`)                                                 | Portrait 390×844; timer 844×390 and design 844×420  | Rebuilding a run on BACK, losing stored bytes, overflowing, or hiding required controls fails route/storage/geometry assertions | AUD-015's selective `RUN_KEY` write failure and retry state                                                                                       |
