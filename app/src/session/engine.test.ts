...
// - Meltemi (O2): wu 10' + 10,000m @ 6k+13 — a single, non-repeated
//   distance work step, for the second hand-pinned nextDistance case.
// - Moderate Breeze (O2): wu 10' + 6'/6'/6' negative-split (spm 18/20/22) —
//   three SEQUENTIAL time work phases with no reps and no auto-rest, ideal
//   for the catch-up walk (each phase's boundary is unambiguous).
...
function meltemiRun(now = t0): SessionRun {
  const d = buildDraft(draftInputFor("Meltemi", `mt-${Math.random()}`));
  return buildRun(d, baselines, tol, now);
}

function moderateBreezeRun(now = t0): SessionRun {
  const d = buildDraft(draftInputFor("Moderate Breeze", `mb-${Math.random()}`));
  return buildRun(d, baselines, tol, now);
}
...
describe("buildRun", () => {
  ...
  it("folds spmOverrides and nudges into the frozen targets, matching what Confirm displayed (Meltemi)", () => {
    const d = buildDraft(draftInputFor("Meltemi", "mt-nudged"));
    ...
    expect(work.targetSplit).toBe(128); // 133 unnudged -> 128 after -5
...
describe("remainingSeconds", () => {
  it("returns the full phase duration at the instant a phase starts", () => {
    expect(remainingSeconds(moderateBreezeRun(), t0)).toBe(600);
  });

  it("decreases by elapsed time within the phase", () => {
    expect(remainingSeconds(moderateBreezeRun(), addSeconds(t0, 120))).toBe(
      480, // 600 (wu) - 120
    );
  });

  it("floors at 0 rather than going negative", () => {
    expect(remainingSeconds(moderateBreezeRun(), addSeconds(t0, 601))).toBe(0);
  });
  ...
  it("is 0 past the last phase", () => {
    const run = { ...moderateBreezeRun(), index: 4 };
...
describe("tick — the catch-up walk", () => {
  ...
  it("walks forward across exactly 3 finished time phases (resilience 3), landing partway into the 4th with phaseStartedAt seeded at the newest boundary, not now", () => {
    const run = moderateBreezeRun();
    // wu 600s + work 360s + work 360s = 1320s boundary; +100s into phase 4.
    const now = addSeconds(t0, 600 + 360 + 360 + 100);
    const result = tick(run, now);
    ...
    expect(remainingSeconds(result, now)).toBe(260); // 360 - 100
  });
  ...
  it("completing the last time phase during a walk sets completedAt to the TRUE finish boundary, not the (much later) wake-up time", () => {
    // Phase 6B Task 1 review, F2: workout actually finishes at t0+1680s
    // (12:28-ish); the phone doesn't wake to recompute until an hour
    ...
    const run = moderateBreezeRun();
    const finishBoundary = addSeconds(t0, 600 + 360 + 360 + 360); // 1680s
...
describe("pause / resume", () => {
  ...
  it("is idempotent: pausing an already-paused run doesn't move pausedAt", () => {
    const first = pause(moderateBreezeRun(), addSeconds(t0, 50));
...
describe("resilience — reload and pause accounting across reloads", () => {
  ...
  it("resilience 1 / 6: reload mid-phase restores the same phase and correct remaining", () => {
    ...
    expect(before).toBe(480); // 600 (wu) - 120
  ...
  it("resilience 2: reload while paused keeps the same remaining time", () => {
    ...
    expect(remainingSeconds(paused, now)).toBe(550); // 600 (wu) - 50
...
describe("advance / rewind", () => {
  it("skips to the next phase, re-seeding its clock at now and clearing pause state", () => {
    const paused = pause(moderateBreezeRun(), addSeconds(t0, 10));
  ...
  it("past the last phase completes the run", () => {
    const run = { ...moderateBreezeRun(), index: 3 }; // the last phase (0-indexed, 4 total)
  ...
  it("steps back to the previous phase, re-seeding its clock", () => {
    const run = { ...moderateBreezeRun(), index: 2 };
    const now = addSeconds(t0, 500);
...
describe("nextDistance", () => {
  ...
  it("records a non-integer average split — hand-pinned: 4567s / 10000m x 500 = 228.35 (Meltemi)", () => {
    const run = { ...meltemiRun(), index: 1 }; // the 10,000m work phase
  ...
  it("is a no-op on a non-distance (time) phase", () => {
    const run = moderateBreezeRun(); // index 0: wu, a time phase
...
describe("isComplete", () => {
  it("is false before the last phase", () => {
    expect(isComplete(moderateBreezeRun())).toBe(false);
  });

  it("is true once index reaches phases.length", () => {
    const run = { ...moderateBreezeRun(), index: 3 };
...
describe("totalRemainingSeconds", () => {
  it("sums remaining time-phase seconds across the current and all upcoming phases (Moderate Breeze)", () => {
    const run = moderateBreezeRun();
    const now = addSeconds(t0, 50); // 50s into the 600s warmup
    // 550 (wu remaining) + 360 + 360 + 360 = 1630
    expect(totalRemainingSeconds(run, now)).toBe(1630);
  });
