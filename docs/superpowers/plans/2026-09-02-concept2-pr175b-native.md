...
```
  ...
  **Permitted lines: step 3b's list, PLUS** the three `browserFinished` occurrences named above — the past-tense narration the Step 3b human read already covers. Any other moved count is still a defect: fix it, or add it to the table with its reason before the PR opens. Paste THIS diff into the PR Record as the pass condition; step 3b's earlier run is kept alongside it only as the preliminary check.
