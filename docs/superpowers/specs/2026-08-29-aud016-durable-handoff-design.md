...
> **SUPERSEDED (2026-08-30).** The carrier substrate this spec designs
> (the module slot, its stash sites, `stillLive`, the save-time carrier
> checks) was replaced by the hand-off store protocol —
> `2026-08-30-handoff-protocol-design.md`, approved by James — after his
> two reviews of PR #230 proved the ownership model underspecified. This
> document REMAINS the record of: the Gate-0-approved held-error frame
> (strings, classes, order — restored verbatim on the fresh branch), the
> `SaveVerdict` four-exit mapping (kept by the protocol), the producer
> honesty section, and the behaviors the protocol preserves. Do not
> implement from its §1/§3/§5 ownership mechanics; the protocol's
> normative sections replace them.
