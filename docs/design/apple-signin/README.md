...
`render.mjs` uses the repository’s installed Playwright 1.62.1 and Chromium. It produces 23 PNGs in `renders/`, including a current/proposed side-by-side, before/after sign-in at 390 × 844 and 844 × 390, native/web welcome variants, signup and missing-email recovery, both You provider states, linking in both directions, success/conflict/expired/account-changed outcomes, and landscape evidence for the Apple-create, You methods, link-confirmation, and account-changed states. `renders/layout-audit.json` records every interactive box and checks for horizontal overflow and targets below 44 × 44 px.
