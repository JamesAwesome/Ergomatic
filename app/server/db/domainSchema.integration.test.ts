...
describe('domain schema against real Postgres', () => {
  ...
  it('rejects a session log with pain outside 1..5', async () => {
    const [u] = await db
      .insert(users)
      .values({ googleSub: 'pain-check-log', email: 'painlog@x.com', name: 'PainLog' })
      .returning()
    await expect(
  ...
  it('cascades: deleting a user removes their workouts, logs, and prefs', async () => {
    const [u] = await db
      .insert(users)
      .values({ googleSub: 'cascade-user', email: 'cascade@x.com', name: 'Cascade' })
      .returning()
    ...
    await db.insert(testHistory).values({ userId: u.id, distance: '2k', splitSeconds: 100 })

    await db.delete(users).where(eq(users.id, u.id))

    expect(await db.select().from(workouts).where(eq(workouts.userId, u.id))).toHaveLength(0)
    expect(await db.select().from(sessionLogs).where(eq(sessionLogs.userId, u.id))).toHaveLength(
      0,
    )
    expect(
      await db.select().from(preferences).where(eq(preferences.userId, u.id)),
    ).toHaveLength(0)
    expect(await db.select().from(baselines).where(eq(baselines.userId, u.id))).toHaveLength(0)
    expect(await db.select().from(planState).where(eq(planState.userId, u.id))).toHaveLength(0)
    expect(
  ...
  it('nulls session_logs.workout_id when the workout is deleted, keeping the log', async () => {
    const [u] = await db
      .insert(users)
      .values({ googleSub: 'setnull-user', email: 'setnull@x.com', name: 'SetNull' })
      .returning()
    ...
    const [after] = await db.select().from(sessionLogs).where(eq(sessionLogs.id, log.id))
