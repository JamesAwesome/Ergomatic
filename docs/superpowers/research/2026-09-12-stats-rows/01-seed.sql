-- Phase PS PR 1 DBA seed. Retargets the 2026-09-07 research generator
-- (docs/superpowers/research/2026-09-07-machine-summary-jsonb-vs-columns/02-gen.sql,
-- function dba_gen_a) at the REAL session_logs table, with the user id as an
-- argument instead of a round-robin over 40 rows.
--
-- Three measured users: dba-1k@test.local (1,000 rows), dba-10k@test.local
-- (10,000), dba-100k@test.local (100,000); filler user dba-fill@test.local
-- carries the remainder to 1,000,000 total rows.
--
-- Diff against 02-gen.sql: `logs_a` -> `session_logs`; `u.id` -> $1 (uid);
-- the `join dba_users` and its `cross join lateral` user pick are dropped
-- (ispm kept); `verified` is left to its column default (NULL) because the
-- generator predates it.

insert into users (email, name) values
  ('dba-1k@test.local', 'dba 1k'),
  ('dba-10k@test.local', 'dba 10k'),
  ('dba-100k@test.local', 'dba 100k'),
  ('dba-fill@test.local', 'dba fill')
on conflict do nothing;

create or replace function dba_gen(uid uuid, lo bigint, hi bigint) returns void language plpgsql as $fn$
begin
...
select dba_gen((select id from users where email = 'dba-1k@test.local'), 1, 1000);
select dba_gen((select id from users where email = 'dba-10k@test.local'), 1, 10000);
select dba_gen((select id from users where email = 'dba-100k@test.local'), 1, 100000);
select dba_gen((select id from users where email = 'dba-fill@test.local'), 1, 889000);

vacuum analyze session_logs;

select count(*) as total from session_logs;
select u.email, count(*) from session_logs l join users u on u.id = l.user_id group by 1 order by 2;
select pg_size_pretty(pg_total_relation_size('session_logs')) as total_size;
