...
describe("generateName", () => {
  ...
  it("gives a different name on each press against the real library", () => {
    // Regression: the generator probed linearly forward from a seed-derived
    // start index. Its noun list opens with the same weather words the
    ...
    const titles = LIBRARY_WORKOUTS.map((w) => w.title);
