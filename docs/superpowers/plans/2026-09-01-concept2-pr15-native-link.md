...
**Spec:** `docs/superpowers/specs/2026-08-31-concept2-logbook-design.md` §Architecture 2-3 + §PR decomposition (PR1.5). **Antagonist pass: <controller fills after the pass>** — fix round 2, P2(i): this PR touches AUTH (the account-injection question the design-gate package is about), which triggers CLAUDE.md's standing TRIAD override regardless of phase position. This revision's earlier text claimed a spoken SKIP ("inherits the phase anchor; no new invariant class in the plumbing"); that no longer applies once AUTH is in scope, and the controller runs the full pass on the revised design separately from this fix round.
