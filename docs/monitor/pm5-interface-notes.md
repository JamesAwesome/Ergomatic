...
**Table 5 — Unique Frame Flags** (CSAFE doc p.8):
...
**Table 6 — Byte Stuffing Values** (CSAFE doc p.8): each occurrence of a flag
...
increase in size by a factor of two in the worst case" (CSAFE doc p.8).
Stuffing applies to the checksum byte too, not only the payload — the
document's own Fixed Distance example response frame is annotated `F3 or 72
Stuff byte flag (checksum = F2) or checksum`, i.e. *if the computed checksum
happens to equal a flag value, the checksum byte itself gets stuffed*
(CSAFE doc p.80).
...
## 6. The nine byte-vector examples (CSAFE doc pp.79–89)

All bytes below are transcribed from the document's own row-by-row command
tables (not its ad hoc summary hex lines, two of which contain independent
transcription slips unrelated to the checksum errata — see the "Fixed
Calorie Interval" note at the end of this section). **Every checksum below
was recomputed independently by XORing the transcribed content bytes**
(everything between the start flag and the checksum byte, i.e. excluding
`F1`, the checksum itself, and `F2`) — the printed document checksum is
reported alongside for comparison, per the errata discipline below.

### 6 good (document checksum matches the XOR rule)
...
| 2 | JustRow | p.80 | `F1 76 07 01 01 01 13 02 01 01 61 F2` | `0x61` |
...
| 5 | Fixed Distance Interval 500m/:30 rest | p.83 | `F1 76 15 01 01 07 03 05 80 00 00 01 F4 04 02 00 1E 14 01 01 13 02 01 01 0A F2` | `0x0A` |
| 6 | Variable Interval Undefined Rest v100m…2 | p.87–88 | `F1 76 45 18 01 00 01 01 08 17 01 04 03 05 80 00 00 00 64 04 02 00 00 06 04 00 00 32 C8 14 01 01 18 01 01 17 01 03 03 05 00 00 00 2E E0 04 02 00 00 06 04 00 00 32 C8 14 01 01 01 01 09 05 05 80 00 00 00 00 13 02 01 01 8F F2` | `0x8F` |

Example #1 (Predefined — Standard List Workout #3) is a **sixth good
example the adversarial review's table did not include** — the review's
table (`spec-review.md` M1) lists only 8 examples (5 good, 3 bad); this
document has a 9th worked example (the "Predefined" public-CSAFE frame,
`CSAFE_SETPROGRAM_CMD` selecting a factory workout) that the review's
...
**Fixed Calorie Interval note:** the document has a tenth worked example
...
own row-by-row byte table in two places (a `3C` rest-duration byte becomes
`0C` in the summary line, and a `0A` printed checksum in the row table
becomes `3F` in the summary line) — this looks like a page-break artifact in
the source document's own layout (a `Revision 0.27 84` footer lands mid-table)
rather than a checksum-rule violation, and is unrelated to the three-errata
class above (M1's failures are internally consistent frames with a wrong
checksum, not internally inconsistent transcriptions). It is excluded from
both tables above rather than guessed at.

## 7. Command IDs used by the six/three examples above
