...
| **RC-13**                                  | **DONE — landed 2026-09-09 (the RC-13 PR).** This row's old opening — _"The avg-pace verdict zero-fires on a rapid re-arm"_ — is the framing the phase-open antagonist pass FALSIFIED, and it is REPLACED rather than annotated, because the row itself recorded two agents misreading it as already fixed in one session. **NEITHER DOOR HAS A SUPPORTED PRODUCER.** `driver.program(` and `driver.beginFreeRow(` have exactly one non-test caller apiece, both in `useMonitorSession.ts`: the hook's `beginFreeRow` returns on `phase === "ended"`, `JustRow.tsx` adds an independent `armedThisStart` latch, and `session.program()`'s one caller (`ConnectedInterstitial.tsx`) is gated on `phase === "pairing"`, which is written only inside `connect()`, which itself refuses when a driver already exists — so `program()` always runs against a freshly minted driver whose `activeRun` is `null`. **AND THE GUARD THAT HOLDS IT HAS A DATE:** the `ended` clause is `8c8fe05e`, **2026-09-01**, "JustRow PR 2 (#259)" — ONE DAY AFTER James gave this order (2026-08-31) — and its own comment says it was added because the e2e flow caught the product re-arming the instant a row ended. His order was correct when he gave it; a different phase shut the door the next day, incidentally. A guard added reactively last week is not an invariant that has held for a year, which is what justified the code. **WHAT REPLACED IT:** both doors now call `drainSummaryReconcile` — settle, not cancel — placed ABOVE `program()`'s per-run reset block, so the settlement runs before `lastWorkStateAverageSplit = null` and the verdict cannot be filed with a false suppression reason; a re-entrant open/replace is refused at the driver itself (`replacingRun`) rather than by the hook's phase guard; and every `summary-reconciled` detail names its RELEASE CAUSE instead of asserting a 3000 ms window closed (which was already untrue on the existing drain paths). **THE LIVE HALF WAS THE CONTAINMENT, NOT THE DRAIN** — see RC-14 | `phase-rc.md`                |
| **RC-13a**                                 | `beginFreeRow()` has NO per-run reset block at all. `program()`'s replacement path resets ~10 fields (`boundaryHalves`, `session`, `refusedKeysLogged`, `clampedKeysLogged`, `splitAvgPaceProvenanceIndex`, `lastEmittedTotals`, `lastLoggedTwd`, `lastWorkStateAverageSplit`, `summarySeen`, `armedWatch`/`armedWatchFired`/`armedWatchRecoveredLogged`); the free-row door resets none of them. Harmless today only because a free row's empty `program.intervals` makes `reconcileSummary` return at its `lastIndex < 0` guard and suppresses the verdict every time. **WHOEVER ADDS A RESET BLOCK THERE MUST PLACE IT BELOW THE SETTLEMENT** (`drainSummaryReconcile("beginFreeRow() replaced this run …")`) or re-introduce RC-13's C1 bug exactly: a settlement that runs after `lastWorkStateAverageSplit = null` files `suppressed — no work-state (0x0032) averageSplit observed this run` for a run that observed one | `phase-rc.md`                |
| **RC-13b**                                 | `drainSummaryReconcile` has no identity guard, unlike both scheduled callbacks (`armSummaryReconcile`'s and `noteTerminateObservations`'s each open `if (activeRun !== run) return;`). It reconciles whatever `activeRun` reads at the moment it runs, not the run whose deadline it is draining. RC-13 made its stated precondition TRUE by emptying the slot at every door, but the precondition is still ARGUED rather than structural. **Binding the run into the slot** — storing `{ run, cancel }` instead of a bare canceller — would make it structural, and is the fix if this is ever taken | `phase-rc.md`                |
| **RC-13c**                                 | `run-replaced` is DEAD CODE from the product's side. It is written only when `program()` replaces a run that is still OPEN; `driver.test.ts` covers it, and nothing in the product produces it — same reachability fact as RC-13's (a driver is minted inside `connect()`, below a guard that refuses when one already exists, so every `program()` runs against an `activeRun` of `null`). Not removed: it is the one lifecycle transition with no event of its own, so if a UI path that re-programs from `ready` is ever added it becomes the only trace of it. Filed so the next reader does not have to re-derive that | `phase-rc.md`                |
| **RC-13d**                                 | **OPEN INFERENCE, deliberately unsettled:** whether a door settlement's `summary-observations` write lands on the OUTGOING record. `applyProducerCommit` keys on `next.startedAt` and a revision ref rather than on `identityRef`, and the hook's handler writes to `runRef.current`, which the hook does not touch before `await driver.program(p)` — **so it probably would**, INFERENCE, not measured. Left open on purpose: the only ordering that exercises it has no producer (RC-13), so a hook-level test of it would prove the consequence of a condition nothing creates. **Whoever adds a UI path that re-programs from `ready` inherits this question** and must settle it before that path ships | `phase-rc.md`                |
| **RC-13e**                                 | `HASH_SUBWINDOW_MS = 200` is SHORTER than the ~270-310 ms 0x0039→0x003F gap measured on the committed captures, which is why the "emit anyway" fall-through exists at all: on the ordinary wire the window expires before the hash arrives and the observations go out without it. RC-13 threaded a release cause through that path and **must not be read as validating the constant**. Not changed, not measured again, and no owner: filed so the number is questioned by the next change that touches it rather than inherited | `phase-rc.md`                |
| **RC-14**                                  | The avg-pace verdict zero-fires on an ORDINARY finish (walk 2026-08-25, W-2). **Distinct from RC-13; do not fold.** Replay through the walk's own commit `c219ee0` DOES produce the verdict, eliminating the wire, the driver's response and ring eviction; **two survivors — it threw, or something outside the driver dropped the entry.** **James, 2026-08-31: do NOT hunt it; INSTRUMENT it** so the next occurrence names which survivor it was, instead of another silent zero. Per RF19, the instrument ships in the same change. **HELD 2026-09-09 — the diagnosis is kept below as history; THE HOLD ITSELF WAS LIFTED the same day, see the end of this row.** The obvious instrument COULD NOT catch the failure it exists for, and shipping it would have discharged this order while printing nothing: `emit` WAS a bare `for (const cb of listeners) cb(e)` with NO error isolation, running the hook's subscribers synchronously on the caller's stack IMMEDIATELY BEFORE the verdict call, so a try/catch around `recordAvgPaceVerdict` could not catch a throw that happened before the call was reached — a THIRD survivor this row's two-survivor enumeration does not contain. The walked ring's `seq 71` (`split-won ... (a 0x0039 was held...)`) is emitted only by the branch that then runs `emit(summaryObservationsEvent(...))`, so that path demonstrably ran on the losing run. The replay missed it because it drives a TEST subscriber set. Still true and still binding: `eventLog.record()` coalesces identical consecutive entries WITHOUT advancing `seq`, so a constant-string marker is RF21-vulnerable, and `avgPaceVerdict.replay.test.ts` holds only the success state. **OPEN QUESTION: ANSWERED 2026-09-09** — the survivor set is three, not two, and the third is a throw inside a subscriber. **NEXT (≤0.25):** none owed before the work; the blocker is gone. When it is taken it must BRACKET THE PAIR — a record at the call site BEFORE `reconcileSummary(...)`, the try/catch around BOTH calls, and a discriminator in every constant detail string. Anything narrower is a green gate over a silent zero. **UNBLOCKED 2026-09-09 by the RC-13 PR, and NOT DISCHARGED BY IT.** What RC-13 supplied: per-listener isolation inside `emit` (each delivery in its own try/catch), so a throw inside a subscriber is CONTAINED — it no longer fails the operation that emitted it — and is RECORDED as a `listener-threw` ring entry naming the event kind and the error. That removes this row's blocker: the third survivor now leaves a trace of its own. **WHAT THIS ROW STILL OWES, unchanged:** the bracketing record at the call site BEFORE `reconcileSummary(...)`, the try/catch around BOTH calls, and a discriminator in every constant detail string. **James's "INSTRUMENT it" order is NOT discharged** | `phase-rc.md`                |
