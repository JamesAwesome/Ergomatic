...
describe('user store against real Postgres', () => {
  ...
  it('creates a user and finds it by google sub', async () => {
    const created = await store.createUser({ googleSub: 'sub-1', email: 'a@x.com', name: 'A' })
    expect(created).toMatchObject({ googleSub: 'sub-1', email: 'a@x.com', name: 'A' })
    const found = await store.findByGoogleSub('sub-1')
    expect(found).toMatchObject({ id: created.id, email: 'a@x.com', name: 'A' })
  })
  ...
  it('updateProfile overwrites email and name for that user only', async () => {
    ...
    const target = await store.createUser({ googleSub: 'sub-3', email: 'c@z.com', name: 'C' })
