...
import type { PlanKey, PlanStateRow, PlanStateStore } from '../stores/planState.js'
import type { PreferencesPatch, PreferencesRow, PreferencesStore } from '../stores/preferences.js'
...
function makeFakeBaselinesStore(): BaselinesStore {
  ...
  return {
    ...
    async put(userId: string, patch: { k2Seconds?: number | null; k6Seconds?: number | null }) {
...
function makeFakeLogsStore(planState: FakePlanStateStore): LogsStore {
  const byUser = new Map<string, Array<LogInput & { id: string; loggedAt: Date }>>()
  return {
...
function makeFakeTestHistoryStore(): TestHistoryStore {
  const byUser = new Map<string, Array<{ id: string; distance: '2k' | '6k'; splitSeconds: number; deltaSeconds: number | null }>>()
  return {
    async list(userId: string) {
      return byUser.get(userId) ?? []
    },
    async append(userId: string, input: { distance: '2k' | '6k'; splitSeconds: number }) {
      const rows = byUser.get(userId) ?? []
      const previous = [...rows].reverse().find((r) => r.distance === input.distance)
      const row = {
        id: crypto.randomUUID(),
        distance: input.distance,
        splitSeconds: input.splitSeconds,
        deltaSeconds: previous ? input.splitSeconds - previous.splitSeconds : null,
      }
      rows.push(row)
      byUser.set(userId, rows)
      return row
...
function fakeSessionStore(): SessionStore {
  const users: Record<string, SessionUser> = { 'token-a': userA, 'token-b': userB }
  return {
    resolveSession: async (token: string) => {
      const user = users[token]
      if (!user) return null
      return { user, expiresAt: new Date(Date.now() + 100_000), refreshed: false }
    },
...
function validWorkoutBody(overrides: Partial<WorkoutInput> = {}): WorkoutInput {
  return {
    ...
    steps: [
      { k: 'wu', minutes: 10 },
      { k: 'w', duration: { kind: 'time', minutes: 20 }, ref: { base: '2k', off: 10 } },
    ],
    ...overrides,
  }
}
...
describe('GET/PUT /api/baselines', () => {
  ...
  it('PUT updates a field and GET reflects it', async () => {
    ...
    const put = await asA(request(app).put('/api/baselines')).send({ k2Seconds: 120 })
  ...
  it('rejects out-of-bounds k2Seconds with 400 + field', async () => {
    const res = await asA(request(appFor(makeStores())).put('/api/baselines')).send({ k2Seconds: 30 })
  ...
  it('rejects out-of-bounds k6Seconds with 400 + field', async () => {
    const res = await asA(request(appFor(makeStores())).put('/api/baselines')).send({ k6Seconds: 999 })
    expect(res.status).toBe(400)
  ...
  it('isTestResult appends to test history for each provided distance', async () => {
    const stores = makeStores()
    const app = appFor(stores)
    await asA(request(app).put('/api/baselines')).send({ k2Seconds: 120, isTestResult: true })
    const history = await stores.testHistory.list(userA.id)
    expect(history).toHaveLength(1)
    expect(history[0]).toMatchObject({ distance: '2k', splitSeconds: 120, deltaSeconds: null })
  })

  it('isTestResult with both fields appends two history rows', async () => {
    const stores = makeStores()
    const app = appFor(stores)
    await asA(request(app).put('/api/baselines')).send({ k2Seconds: 120, k6Seconds: 130, isTestResult: true })
    const history = await stores.testHistory.list(userA.id)
...
describe('workouts CRUD', () => {
  ...
  it('POST creates a workout and it appears in the list', async () => {
    const app = appFor(makeStores())
    const created = await asA(request(app).post('/api/workouts')).send(validWorkoutBody())
  ...
  it('POST rejects an invalid workout with 400', async () => {
    const res = await asA(request(appFor(makeStores())).post('/api/workouts')).send(
      validWorkoutBody({ steps: [] }),
    )
    expect(res.status).toBe(400)
  ...
  it('POST 409s on a num clash for the same user', async () => {
    const app = appFor(makeStores())
    await asA(request(app).post('/api/workouts')).send(validWorkoutBody({ num: 5 }))
    const clash = await asA(request(app).post('/api/workouts')).send(validWorkoutBody({ num: 5, title: 'Other' }))
  ...
  it('the same num does not clash across users', async () => {
    const app = appFor(makeStores())
    await asA(request(app).post('/api/workouts')).send(validWorkoutBody({ num: 5 }))
    const res = await asB(request(app).post('/api/workouts')).send(validWorkoutBody({ num: 5 }))
  ...
  it('PUT /:id updates the workout', async () => {
    const app = appFor(makeStores())
    const created = await asA(request(app).post('/api/workouts')).send(validWorkoutBody())
    const res = await asA(request(app).put(`/api/workouts/${created.body.id}`)).send(
      validWorkoutBody({ title: 'Renamed' }),
    )
    expect(res.status).toBe(200)
  ...
  it('PUT /:id 404s on a foreign id rather than leaking a 400/409', async () => {
    const app = appFor(makeStores())
    const created = await asA(request(app).post('/api/workouts')).send(validWorkoutBody())
    const res = await asB(request(app).put(`/api/workouts/${created.body.id}`)).send(validWorkoutBody())
    expect(res.status).toBe(404)
  })
  ...
  it('PUT /:id 409s on a num clash with a sibling workout', async () => {
    const app = appFor(makeStores())
    await asA(request(app).post('/api/workouts')).send(validWorkoutBody({ num: 1 }))
    const second = await asA(request(app).post('/api/workouts')).send(validWorkoutBody({ num: 2 }))
    const res = await asA(request(app).put(`/api/workouts/${second.body.id}`)).send(
  ...
  it('DELETE /:id removes the workout', async () => {
    const app = appFor(makeStores())
    const created = await asA(request(app).post('/api/workouts')).send(validWorkoutBody())
    const res = await asA(request(app).delete(`/api/workouts/${created.body.id}`))
    expect(res.status).toBe(204)
    const after = await asA(request(app).get(`/api/workouts/${created.body.id}`))
  ...
  it('DELETE /:id 404s on a foreign id (does not delete it)', async () => {
    const app = appFor(makeStores())
    const created = await asA(request(app).post('/api/workouts')).send(validWorkoutBody())
    const res = await asB(request(app).delete(`/api/workouts/${created.body.id}`))
    expect(res.status).toBe(404)
...
describe('POST /api/workouts/bulk', () => {
  it('requires a text field', async () => {
    const res = await asA(request(appFor(makeStores())).post('/api/workouts/bulk')).send({})
    expect(res.status).toBe(400)
  })

  it('creates every workout in a valid multi-block paste', async () => {
    const text = `1 | Ladder | AT | medium | 3\nwu 10\nw 1' 6k-2 @22 r5\n\n2 | Steady | O2 | easy | 1\nwu 10\nw 20' 2k+10`
    const res = await asA(request(appFor(makeStores())).post('/api/workouts/bulk')).send({ text })
    expect(res.status).toBe(200)
  ...
  it('reports a per-line num-clash without failing the rest of the batch', async () => {
    ...
    const res = await asA(request(app).post('/api/workouts/bulk')).send({ text })
    expect(res.body.created).toHaveLength(1)
...
describe('GET/POST /api/logs', () => {
  const validLogBody = () => ({
    ...
    pain: 2,
    notes: null,
    steps: [{ label: 'Work', targetSplit: 120, actualSplit: 121, actualSource: 'stopwatch' }],
  })
  ...
  it('POST creates a log, freezing current baselines, and it is listed after', async () => {
    ...
    const created = await asA(request(app).post('/api/logs')).send(validLogBody())
    expect(created.status).toBe(201)

    const list = await asA(request(app).get('/api/logs'))
    expect(list.body).toHaveLength(1)
    expect(list.body[0]).toMatchObject({ baselineK2: 120, baselineK6: 130, workoutTitle: 'Steady State' })
  })
...
describe('GET/PUT /api/plan', () => {
  ...
  it('PUT selects a plan and returns its 84-entry sequence', async () => {
    const res = await asA(request(appFor(makeStores())).put('/api/plan')).send({ planKey: 'sprint' })
    expect(res.status).toBe(200)
    expect(res.body.planKey).toBe('sprint')
    expect(res.body.doneN).toBe(0)
    expect(res.body.sequence).toHaveLength(84)
    expect(res.body.sequence[0]).toEqual({ index: 0, code: PLANS.sprint.sessions[0], status: 'today' })
  })

  it('PUT rejects an unknown planKey with 400', async () => {
    const res = await asA(request(appFor(makeStores())).put('/api/plan')).send({ planKey: 'marathon' })
    expect(res.status).toBe(400)
  ...
  it('re-PUTting the SAME planKey preserves doneN and does not call set()', async () => {
    ...
    const res = await asA(request(app).put('/api/plan')).send({ planKey: 'sprint' })
  ...
  it('PUTting a DIFFERENT planKey resets doneN to 0 and calls set()', async () => {
    ...
    const res = await asA(request(app).put('/api/plan')).send({ planKey: 'head' })
...
describe('GET/PUT /api/prefs', () => {
  ...
  it('PUT updates a field and GET reflects the merge', async () => {
    const app = appFor(makeStores())
    const put = await asA(request(app).put('/api/prefs')).send({ accentColor: '#00ff00' })
    expect(put.status).toBe(200)
  ...
  it('rejects a malformed accentColor with 400 + field', async () => {
    const res = await asA(request(appFor(makeStores())).put('/api/prefs')).send({ accentColor: 'red' })
    expect(res.status).toBe(400)
  ...
  it('rejects an out-of-range timeCapMinutes with 400 + field', async () => {
    const res = await asA(request(appFor(makeStores())).put('/api/prefs')).send({ timeCapMinutes: 5 })
    expect(res.status).toBe(400)
    expect(res.body.field).toBe('timeCapMinutes')
  })

  it('rejects an out-of-range warmupMinutes with 400 + field', async () => {
    const res = await asA(request(appFor(makeStores())).put('/api/prefs')).send({ warmupMinutes: 100 })
    expect(res.status).toBe(400)
    expect(res.body.field).toBe('warmupMinutes')
  })

  it('rejects a non-boolean warmupOverride with 400 + field', async () => {
    const res = await asA(request(appFor(makeStores())).put('/api/prefs')).send({ warmupOverride: 'yes' })
    expect(res.status).toBe(400)
    expect(res.body.field).toBe('warmupOverride')
  })

  it('rejects an out-of-range countdownSeconds with 400 + field', async () => {
    const res = await asA(request(appFor(makeStores())).put('/api/prefs')).send({ countdownSeconds: 999 })
    expect(res.status).toBe(400)
    expect(res.body.field).toBe('countdownSeconds')
  })

  it('rejects an out-of-range paceToleranceSeconds with 400 + field', async () => {
    const res = await asA(request(appFor(makeStores())).put('/api/prefs')).send({ paceToleranceSeconds: 99 })
    expect(res.status).toBe(400)
...
describe('GET /api/test-history', () => {
  it('starts empty', async () => {
    const res = await asA(request(appFor(makeStores())).get('/api/test-history'))
    expect(res.status).toBe(200)
    expect(res.body).toEqual([])
  })
...
describe('GET /api/today', () => {
  ...
  it('recommends a matching-type workout from the library', async () => {
    ...
    const created = await asA(request(app).post('/api/workouts')).send(
      validWorkoutBody({ type: todayCode as 'AN' | 'O2' | 'AT' | 'TR', num: 1 }),
    )
    const res = await asA(request(app).get('/api/today'))
