 ...
 *
 *  Phase WU: there is no warm-up phase any more. `buildRun` used to prepend
 *  the rower's warm-up SETTING as a `type: "warmup"` phase (plus an optional
 *  trailing rest) ahead of the expanded steps; that concept is gone from
 *  every union, so a run's phases are exactly what the draft's steps expand
 *  to. */
