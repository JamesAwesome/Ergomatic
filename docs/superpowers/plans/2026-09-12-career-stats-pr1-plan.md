...
| `domain/stats/rowContribution.ts` (new) | `StatsRowInput`, `RowContribution` (discriminated by tier), `rowContribution`, `isReconstructableClose`, `stepActualSums` |
...
| `src/log/storedSummary.ts` (modify) | `toStatsRowInput` + `buildHeroes` calls `rowContribution`; local `isReconstructableClose` deleted |
...
| `eslint.config.js` (modify) | `domain/** → src/**` forbidden |
...
| `StatsScreen.custom` (`{from,to}` strings) | first render of CUSTOM (seeded today−29..today) | unmount | nothing |
...
| `process.env.TZ` in `useStatsRows.tz.test.ts` | file top, before any `Date` | the worker process ends | nothing (its own file, own worker) |
...
```js
  {
    ...
    // logic and may never import src/**. Tests are exempt, as in the src
    // block above: domain/monitor/nfc.test.ts already reads a fixture from
    // src/monitor/nfc/ (the unexempted rule is red on main).
    files: ["domain/**/*.ts"],
    ignores: ["domain/**/*.test.ts"],
    rules: {
...
- [ ] **Step 4:** run the domain test → PASS (measured 13/13).
...
- [ ] **Step 6: Refactor `storedSummary.ts`.** Add `import { rowContribution, type StatsRowInput } from "../../domain/stats/rowContribution.js";`. Delete the local `isReconstructableClose` (its only call is inside `buildHeroes`; leave the comments that cite it, they describe the rule). Add:
...
Mount in `app.ts` inside `if (deps.stores) { … app.use(createStatsRouter({ logs: deps.stores.logs, requireUser: requireUser(deps.sessions) })); }` (import at top). Route test → PASS (measured 4/4; `data.test.ts` + contracts fake still green, 522 tests).
...
- [ ] **Step 6: Mutation (§8.2 (4)).** In `toStatsRow` replace `workMeters: c.workMeters` with `workMeters: r.distanceMeters`; rerun the integration test. Expected: `machineFused` fails `expected 620 to be 500` (the fused figure = machine + 120) — the exact line goes in the PR body. Restore.
...
- [ ] **Step 1: Failing TZ test** — `useStatsRows.tz.test.ts` starts with `process.env.TZ = "America/New_York";` BEFORE the imports; cases: `new Date("2026-09-12T12:00:00Z").getTimezoneOffset()` is `240` (the zone took — RF38); `toCalendarDate(new Date("2026-09-12T23:30:00Z"))` is `{ y: 2026, m: 9, d: 12 }` (UTC would say 13); `toCalendarDate(new Date(2026, 0, 17, 12))` is `{ 2026, 1, 17 }` (month is 1-based). Plus `useStatsRows.test.ts` (the `useRecentLogs.test.ts` idiom: `vi.resetModules`, `vi.doMock("../api")`, `renderHook`): with `vi.useFakeTimers({ toFake: ["Date"] }); vi.setSystemTime(new Date(2026, 8, 12, 9))` a ready state carries `today {2026,9,12}`, the row's `date.y 2026`, and `api` was called with `"/api/stats/rows"`; a 500 yields `error` whose `retry()` fires a second request. → FAIL (module missing).
- [ ] **Step 2: Implement:**
...
- [ ] **Step 3: Gates + mutation.** `… vitest run --project client src/api/useStatsRows.test.ts src/api/useStatsRows.tz.test.ts` → PASS (measured, with `stackedBar.test.ts` and `You.test.tsx`: 4 files, 21 tests). Mutation (§8.3 (4)): swap the three getters for `getUTCFullYear/getUTCMonth/getUTCDate` → the 23:30 case fails `expected { d: 13 } … { d: 12 }`; paste it; restore.
...
- [ ] **Step 7: CSS** (append to `index.css`; the contrast numbers from `contrast.json` in the block comment): `.you-stats-hero { display:block; margin-top:22px; padding:16px; min-height:var(--tap); box-sizing:border-box; background:var(--surface); border:1px solid var(--rule); color:var(--ink); text-decoration:none }` `.you-stats-hero:focus-visible { outline:2px solid var(--ink); outline-offset:2px }` `.you-stats-figures { display:flex; flex-wrap:wrap; justify-content:space-between; gap:4px 12px; margin:0 0 12px; font-family:var(--font-mono); font-size:12px; font-weight:600; letter-spacing:.08em; font-variant-numeric:tabular-nums }` `.stacked-bar { display:block; width:100%; height:24px }` `.stacked-bar-seg-an{fill:var(--type-an)} …-at{fill:var(--type-at)} …-o2{fill:var(--type-o2)} …-tr{fill:var(--ink)} …-none{fill:var(--ink-4)}` `.stats-legend-line,.stats-caption,.stats-note { margin:8px 0 0; font-family:var(--font-mono); font-size:11px; letter-spacing:.06em; color:var(--ink-3) }` `.stats-legend-line{color:var(--ink)}` `.stats-caption-quiet{color:var(--ink-4)}`.
- [ ] **Step 8: Gates + mutation.** Hero test → PASS (measured 4/4). Mutation (invariant 16): wrap `<TypeLegend …/>` in a second `<Link to="/you/stats">` → `× is exactly ONE focusable control … expected <a href="/you/stats" …> to have a length of +0 but got 1` (measured). Restore. Contrast: `--ink` on `--surface` 17.11:1, `--ink-3` on `--surface` 7.43:1 (`contrast.json`).
...
function Body({ rows, today, preset, onPreset, custom, onCustom, applied }: BodyProps) {
  ...
  const customInvalid = preset === "custom" && from !== null && to !== null && customRange(from, to) === null;
  ...
  return (<>
    <StatsFilterBar preset={preset} onPreset={onPreset} custom={custom} onCustom={onCustom} customInvalid={customInvalid} />
    {inRange.length === 0
...
```ts
export async function backdateLog(id: string, date: string, zone: string): Promise<void> {
  ...
  try {
    const res = await client.query("update session_logs set logged_at = ($2::timestamp) at time zone $3 where id = $1", [id, `${date} 12:00:00`, zone]);
    if (res.rowCount !== 1) throw new Error(`backdateLog: expected 1 row for ${id}, got ${res.rowCount}`);
  } finally { await client.end(); }
}
```

Defaults mirror `compose.yml` and CI's e2e job (which exports only `POSTGRES_PASSWORD=devpass`); `scripts/stack-env.sh` exports the per-worktree `POSTGRES_PORT` locally.
- [ ] **Step 3: `e2e/stats.spec.ts`** — `test.use({ timezoneId: "America/New_York" })`; `page.clock.install({ time: new Date("2026-09-12T09:00:00") })`; sign in (`stats-${RUN_ID}@e2e.test`); `seedGate0(page)` posts each body via in-page `fetch`, `backdateLog`s it, and ASSERTS the backdate took: `GET /api/logs/${ids.R13}` → `loggedAt === "2026-09-11T16:00:00.000Z"` (noon EDT); `/you`: `getByRole("link", { name: "Stats" })` contains `LIFETIME · 56,752 M` and `SEASON 2027 · 43,012 M`; `.you-doors > *` count 3 — BASELINES · SETTINGS · DIAGNOSTICS; the CONCEPT2 row renders only for an account whose read says `available: true`, and the e2e stack sets no `C2_LINK_ENABLED` (a first run asserted 4 and read 3, measured) — and no `STATS`; focus `Sign out`, `Tab` → hero focused; click `hero.locator(".stats-legend-line")` → URL `/you/stats`; rows (`getByRole("row", { name: /^METRES/ }).getByRole("cell").nth(0|1)`) `56,752`/`36,752`, `8 OF 10 CARRY THE MONITOR'S OWN TOTALS`, REST `718`, CALORIES `1,731`, AVG WATTS `176`, the singular seam line; then `/today/log/${ids.R13}` → `Delete session` → `.log-delete-confirm` `Delete session`; `/you` → `LIFETIME · 54,752 M`; `/you/stats` → MACHINE `34,752` and `7 OF 9 CARRY THE MONITOR'S OWN TOTALS`. Run `pnpm e2e e2e/stats.spec.ts` against the booted stack. Mutation (§8.5): make `summarize`'s ALL column filter `source === "pm5"` → the `56,752` assertion fails while `36,752` passes (rebuild: `pnpm e2e` builds; confirm `pnpm build` succeeds first, RF12 corollary).
...
- [ ] **Step 1:** Commit the scripts: `README.md` (the six steps); `01-seed.sql` (three users `dba-1k/10k/100k@test.local`, `dba_gen_a` from the 2026-09-07 dir retargeted at `session_logs` — paste its body with `logs_a → session_logs`, `u.id →` the user id argument — at 1k/10k/100k plus filler to 1M, `vacuum analyze`); `queries.sh` (`Q_STATS` = the SHIPPED query text, verbatim from `STATS_ROW_COLUMNS`, no ORDER BY, with a `%s` user id); `bench.sh` (six runs per user in one psql session via `printf '%s\n' '\timing on'` — NOT `echo`, dba-techniques step 4 — discard run 1, median of 5, `BENCH_PRE` honoured); `explain.sh` (`explain (analyze, buffers)` per user); `payload.sh` (backdoor sign-in, `curl … -w '%{size_download} B %{time_total} s'` with and without `Accept-Encoding: gzip`, bytes/row via node). All `bash -n` clean (measured).
...
3. **`isReconstructableClose` moves into the domain; `storedSummary.ts` keeps its own `stepActualSums`** (the B2 TOTAL line still needs the `{meters?,seconds?}` shape). Two sum loops, one contract test holding them equal.
...
6. **The domain→src ESLint rule exempts `domain/**/*.test.ts`**: `domain/monitor/nfc.test.ts:8` imports `../../src/monitor/nfc/fixtures`, so the unexempted rule is red on main (measured). Tests are not in the bundle.
7. **Two hero states Gate 0 did not draw:** loading reads `LOADING…`; error reads `COULDN'T LOAD STATS · TAP TO OPEN`; at 0 rows the figures read `0 M` and the §5 `NO ROWS YET …` string stands where the bar would be. The custom-range empty state's dates print as `YYYY-MM-DD` (the input's own format). All four go to James at the PR as copy for approval.
...
Every block above was written to its REAL path in the worktree at `c5cfab24`, then removed (nothing committed). Commands and results:
...
- `vitest --project unit domain/stats` → 4 files / 28 tests; `server/routes/stats.test.ts` + `contracts.fake.test.ts` + `data.test.ts` → 522 tests; `--project integration server/routes/stats.integration.test.ts` → 1/1 (Docker, ~40 s).
- `vitest --project client`: `heroesContract.test.ts` 13/13; `storedSummary.test.ts` + `FromTheLog.test.tsx` + `logbookDerived.test.ts` 344/344 after the refactor; `useStatsRows.test.ts` + `.tz.test.ts` + `stackedBar.test.ts` + `You.test.tsx` 21/21; `YouStatsHero.test.tsx` 4/4; `StatsScreen.test.tsx` 9/9; `concept2Independence.test.ts` 4/4; `gate0LogBodies.test.ts` 13/13.
- `pnpm exec tsx scripts/capture-heroes.ts` → the six-row JSON quoted in Task 2 (on the pre-refactor tree).
- Mutations run and restored: gate swap (Task 2), `<`/`<=` and all-buckets (Task 3), nested Link (Task 6), ESLint import (Task 1) — failure texts quoted where they appear. Not run: the route `distanceMeters` mutation, the getters→UTC mutation, the Task 7 trio, the Task 8 pair, the e2e ALL-column mutation — each is prescribed with its expected line and must be run at implementation.
- `pnpm e2e e2e/stats.spec.ts` against the per-worktree stack (`ergomatic-68485`, web :8185, pg :15185): first run failed only on `.you-doors` count (4 asserted, 3 rendered — no CONCEPT2 row in the e2e stack; fixed as Task 9 says); second run `1 passed (1.1s)` — seed, backdate (R13 `2026-09-11T16:00:00.000Z`), hero figures, Tab focus, legend click, both columns, delete R13 → `54,752` / `34,752` / `7 OF 9`. The stack is left up (`E2E_KEEP` default) for the implementer.
