...
## Revision · 2026-08-11 — panes cut to two, live pane rebuilt

Supersedes sections 3 and 4 below wherever they disagree. Everything else stands.

- **Connected mode has two panes: LIVE and GRID.** The phone timer pane is dropped —
  with the erg counting, it was a second reading of the same clock. Interval countdown
  now lives in the live pane's metric row and in the grid header. The phone timer is
  unchanged for unconnected sessions.
- **Live is split and rate, both actual and target, both large.** Actuals 112px
  landscape / 104px portrait; each target directly beneath at 38px in ink. The 10px
  `TARGET 24` caption is gone — it was the reason target rate was unreadable at speed.
- **The hero can no longer clip.** Tenths at half size, `white-space:nowrap`, column
  has `min-width:0`. Cap the split at m:ss.t; slower than 9:59.9 renders `—`.
- **The pane rail lives inside the landscape sensor gutter** (44pt, reserved anyway):
  one dot above the camera housing, one below. Content is no longer inset to clear it.
  Portrait keeps a 54px two-tab bar.
- **End session is a 44pt outlined `END` in the header**, not a full-width button.
  Staging unchanged.
- **Grid rows are single-line, fixed height** (32px landscape, 34px portrait):
  8 rows visible landscape, 12 portrait. Active row is a filled surface between two ink
  rules with a 4px marker — no card padding. Totals moved into the header line.
