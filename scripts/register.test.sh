...
has "$out" "register open:6 closed:4 sub:1" "count: register tallies both carriers"
...
lacks "$out" "A live row that names no code site" "closed: does NOT flag an open [ ] row"
...
has "$out" "register base:6 head:6 delta:0" "ratchet: reports base, head and delta"
teardown
