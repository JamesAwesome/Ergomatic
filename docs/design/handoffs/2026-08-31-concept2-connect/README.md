...
States: idle (Send action) → sending → **sent** (persists forever; the row
carries Concept2's own result id) / **duplicate** ("Concept2 already has
this row" — their logbook rejected it as matching an existing row's date,
time and distance) / **failed** (retryable). Also decide the treatment for
non-qualifying rows and for the not-linked case (absence vs disabled-with-
why; the spec leaves this to design).
