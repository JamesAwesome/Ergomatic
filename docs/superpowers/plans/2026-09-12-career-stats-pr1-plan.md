...
- [ ] **Step 3: Gates + mutation.** `… vitest run --project client src/api/useStatsRows.test.ts src/api/useStatsRows.tz.test.ts` → PASS (measured, with `stackedBar.test.ts` and `You.test.tsx`: 4 files, 22 tests). Mutation (§8.3 (4)): swap the three getters for `getUTCFullYear/getUTCMonth/getUTCDate` → the 23:30 case fails `expected { d: 13 } … { d: 12 }`; paste it; restore.
