...
select gen_random_uuid(),'030c1d9e-4787-4d73-8f1b-ee2ad4ce7704','WALbench','O2',now(),
...
\echo '--- b insert (run 1) ---'
explain (analyze, wal, costs off, timing off, summary off)
insert into logs_b (id,user_id,workout_title,workout_type,logged_at,steps,source,held,effort,
  avg_split_seconds,distance_meters,time_seconds,ended_by,work_seconds,work_meters,rest_seconds,rest_meters,
  machine_work_seconds,machine_work_meters,machine_summary,completed_at,tz,machine_calories,machine_avg_watts,machine_cal_per_hour,machine_rest_meters)
select gen_random_uuid(),'030c1d9e-4787-4d73-8f1b-ee2ad4ce7704','WALbench','O2',now(),
  (select jsonb_agg(jsonb_build_object('label','Interval '||i,'targetSplit',112.0,'actualSplit',111.6,
    'actualSource','pm5','spm',24,'meters',2000,'seconds',480,'avgHr',151,'actualSeconds',479.4,
    'actualMeters',2000,'actualSpm',23,'machineCalories',53,'machineCalPerHour',842,'machineWatts',162,
  ...
  jsonb_build_object('avgStrokeRate',24,'endingHeartRateBpm',148,'avgHeartRateBpm',152,'minHeartRateBpm',98,
    'maxHeartRateBpm',171,'dragFactorAverage',118,'workoutType',1,'recoveryHeartRateBpm',120,
    'avgPaceSecondsPer500m',111.8,'verificationBytes',(select jsonb_agg(b) from generate_series(1,19) b)),
...
select gen_random_uuid(),'030c1d9e-4787-4d73-8f1b-ee2ad4ce7704','WALbench','O2',now(),
  ...
  jsonb_build_object('avgStrokeRate',24,'endingHeartRateBpm',148,'avgHeartRateBpm',152,'minHeartRateBpm',98,
    ...
    'avgPaceSecondsPer500m',111.8,'verificationBytes',(select jsonb_agg(b) from generate_series(1,19) b)),
  now(),'Europe/London',372,162,858,274;
\echo '--- update: 1000 rows, four fields ---'
explain (analyze, wal, costs off, timing off, summary off)
...
where id in (select id from logs_a where source='pm5' order by id limit 1000);
explain (analyze, wal, costs off, timing off, summary off)
update logs_b set machine_calories=373, machine_avg_watts=163, machine_cal_per_hour=859, machine_rest_meters=275
where id in (select id from logs_b where source='pm5' order by id limit 1000);
explain (analyze, wal, costs off, timing off, summary off)
update logs_a set machine_summary = machine_summary || jsonb_build_object(
  'totalCalories',373,'avgWatts',163,'avgCalPerHour',859,'totalRestMeters',275)
where id in (select id from logs_a where source='pm5' order by id limit 1000);
