...
(offset 16) was APPENDED to this characteristic in interface-definition
revision V1.26 — rev 1.30 Table 1, verbatim: "Added Erg Machine Type
parameter to characteristic 0x0032/0x0080/ V1.26.", dated 11/2/2018. A
monitor on older firmware sends a 16-byte prefix ending at Rest Time;
`parse.ts` floors this characteristic at 16 bytes, not 17, for exactly that
reason (`ADDITIONAL_STATUS_1_MIN_BYTES`). The 17-byte figure in this table's
own heading is still the full documented length — it is silent about the
floor, and that silence is what a strict 17-byte check read as correct.
...
below. **MEASURED against our own corpus, not assumed:** across all 20
files in `docs/monitor/sessions/`, 0x0032 is 17 bytes in 8248 of 8248
notifications and 0x0038 is 19 bytes in 42 of 42 — every monitor we have
ever captured is post-V1.26, so no capture we hold exercises the short
form and the floor is unverified against real pre-2018 hardware.
...
**FIRMWARE-DEPENDENT TRAILING FIELD, one revision later.** Same mechanism as
0x0032's note above: `Erg Machine Type` (offset 18) was appended in V1.27
(11/8/2018 — rev 1.30 Table 1, verbatim: "Added Erg Machine Type parameter
to characteristic 0x0038."), so a pre-V1.27 monitor sends an 18-byte prefix
and `parse.ts` floors this characteristic there
(`ADDITIONAL_SPLIT_INTERVAL_MIN_BYTES`), not at the 19-byte full length this
table's heading states.
