...
Candidate: `259882ba` in `.claude/worktrees/wave-a-apple-server-paste`, based on `3cf849c7`. Source commits are `82fb92cb` (candidate), `299a31d3` (session-first locking), `f31b4190` and `832cde62` (producer/cleanup/input/freshness gates), `16b659f1` (untrusted signing-key rejection), `0f4921d8` (shared HTTP start boundary), and `259882ba` (stage-owned failure cleanup and HTTP response-loss producer). The server plan's complete patches are generated from that committed range. Parent branch/native/client/deployment changes are not included in this source claim.
...
## F1 mechanism correction and F4 producer receipt

At `259882ba`, `Attempts.discard(expected)` deletes only the exact stage/version plus immutable binding, surface, purpose, provider intent, state, nonce and original-session snapshot. It returns whether a row was erased. The callback updates its cleanup snapshot only after a successful claim. Provider cancellation, malformed proof/provider error, and losing claim failures use the same conditional cleanup; only a successful owned deletion clears the attempt cookie. Native failed exchanges likewise discard their claimed snapshot. Explicit holder `/cancel` remains operation-wide. A cleanup database error preserves the binding cookie and does not leak its exception. No provider I/O was moved under locks.

The reviewer’s `/tmp/apple-mechanism-probes/server.ts` was executed verbatim, with a new isolated PostgreSQL 18.4 container bound to the same port 62918. Before correction its winner had `confirm/version3/grant=true`, then the loser left no row (`f1-reviewer-red.log`). After correction the same probe retained `confirm/version3` after the loser (`f1-reviewer-green.log`). The exact probe is preserved as `f1-reviewer-probe.ts`; its provider verifier is synthetic because the deciding mechanism is callback ownership. The author container was stopped and automatically removed.

Before implementation, eight new HTTP→real-read/claim/accept→PG regressions failed on winner erasure (`f1-tests-red.log`). Both callbacks read the same actual authorization snapshot; the loser is held until the winner advances. The matrix covers signup→confirm and Apple reauth→Google target, crossed with a second valid callback, provider cancellation, provider error and malformed code. Corrected tests assert the winning stage/version/pending grant survives, no stale cookie clear is sent, resume works and the winner completes account creation or both-method linking. Separate tests prove truly owned malformed/provider/exchange failures erase immediately, native claimed cleanup works, cleanup failure preserves its cookie, and existing explicit holder cancellation still works.

The F4 producer test `finalize can commit identity and grant before its HTTP response is lost` prepares a link through real store transitions, POSTs the real `createApp` finalization route with a real PG-backed session, and destroys response delivery only when the route emits `linked` after its commit. Supertest receives `socket hang up`; the actual methods endpoint then reports both providers, the real grant persists and the attempt is consumed. It proves the server/transport seam, not client rendering or a physical network incident. The client author owns the uncertainty-copy consumer test and mutation; no production fake-provider seam was introduced.

Executed correction gates: `pnpm test --project integration server/auth/frontDoorRoutes.integration.test.ts server/auth/attempts.integration.test.ts` PASS: 47 tests/2 files (`f1-tests-green.log`). `pnpm lint`, `pnpm typecheck`, `pnpm format:check` PASS (`f1-lint.log`, `f1-typecheck.log`, `f1-format.log`); commit 259882ba again ran actual staged Prettier/ESLint and whole-project typecheck (`f1-commit.log`). Initial test typing used an unsupported TS lib spelling and omitted a receiver annotation; both were corrected before commit. No full unchanged suite was run.

After confirmed clean commit 259882ba, the executed deciding mutations were `failure-id-only-cleanup` (eight concurrency tests fail), `failure-stale-cookie`, `failure-claimed-snapshot`, `native-failure-cleanup`, `failure-cleanup-error-cookie`, `lost-response-grant`, and the updated `callback-cancel-erasure`. Each produced assertion failures and restored committed source; commands, exact HEAD and failure counts are in `mutations.json` and individual logs. The final restored coverage run again passed all 47 tests.

Per-file coverage was read from the authoritative HTML for the narrow two-file command: `pnpm test:coverage --project integration server/auth/frontDoorRoutes.integration.test.ts server/auth/attempts.integration.test.ts --coverage.include=server/auth/attempts.ts --coverage.include=server/auth/frontDoorRoutes.ts`. This command exited 1 because scoped aggregate statements 89.90% and branches 89.30% miss 90%; functions 98.18%, lines 91.89%. It is not a passing whole-branch coverage claim. No threshold was changed and unrelated unit suites were not added to this correction run. HTML is retained as `f1-coverage-index.html` with file pages and extracted `f1-coverage.json`.

| Changed production file | Statements | Branches | Functions | Lines |
|---|---:|---:|---:|---:|
| `server/auth/attempts.ts` | 89.17% | 89.06% | 96.96% | 92.41% |
| `server/auth/frontDoorRoutes.ts` | 90.62% | 89.56% | 100% | 91.39% |

DBA remeasurement delta: only `server/auth/attempts.ts` gains one PK-bounded conditional DELETE. SHA256 `d8df3812f5b4bb6e29256f3ced465616f3b0cf3b9d51582b6a90657772a88980`; routes SHA256 `41d1beb1c3b94e5a1b25748dbb1180c3dfd55d0fba3f776030963f02ab3ff512`. Existing claim/session lock ordering, migration/schema and prior query bodies are unchanged. The new DELETE only touches the attempt row; its scale/lock measurement belongs to the later DBA delta receipt. Parent owns lens 2 and assembled release checks. The mechanism is closed by the producer regressions; no repeat lens 1 is claimed.

## Per-file coverage

Numbers below are historical scoped coverage at `16b659f1`, before later tests and F1 cleanup. They are not current coverage for the two production files changed by F1; their correction-run rows appear immediately above. They were read from the actual HTML indexes, including fully covered files omitted by the text summary. The original HTML indexes are retained as `server-auth-coverage.html` and `server-coverage.html`; `coverage.json` is their mechanically extracted table. `server/index.ts` is excluded by the repository's existing coverage configuration: its wiring is compile/source evidence, not a booted-server claim. Shared types have no runtime coverage. Schema/migration behavior is exercised by real-Postgres fixtures and DBA queries.
...
|---|---|---|
| Losing callback/failure cleanup cannot erase later authority or its cookie | Held real HTTP callback reads across signup and reauth wins; pending grant survives and completion succeeds; owned failures still clean immediately | `failure-id-only-cleanup`, `failure-stale-cookie`, `failure-claimed-snapshot`, `native-failure-cleanup`, `failure-cleanup-error-cookie` |
| Missing finalization response is not evidence of rollback | Real PG commit followed by socket destruction, then actual methods/grant reads | `lost-response-grant`; client owns copy mutation |
