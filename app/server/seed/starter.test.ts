...
describe('STARTER_WORKOUTS', () => {
  ...
  it('nums are exactly 1..N with no gaps or duplicates', () => {
    ...
    expect(nums).toEqual(Array.from({ length: STARTER_WORKOUTS.length }, (_, i) => i + 1))
  })
  ...
  it('covers every type × difficulty combination at least twice', () => {
    for (const t of TYPES) {
      for (const d of DIFFS) {
        const n = STARTER_WORKOUTS.filter((w) => w.type === t && w.difficulty === d).length
  ...
  it('fills every time band with at least 6 workouts at reference baselines', () => {
    const counts: Record<string, number> = { '<30': 0, '30-45': 0, '45-60': 0, '60+': 0 }
    for (const w of STARTER_WORKOUTS) {
  ...
  it('assigns pain sensibly: easy O2 stays low, hard AN/TR stays high', () => {
    for (const w of STARTER_WORKOUTS) {
      if (w.type === 'O2' && w.difficulty === 'easy') expect(w.pain, w.title).toBeLessThanOrEqual(2)
