...
| New handoff's SPM stepper: `+` from empty jumps to **18** ("Interactions & behaviour") | Ergomatic's SPM stepper (`StepEditor.tsx`, built on the shared `Stepper.tsx`) wakes at **20** instead, and clamps `10..60` rather than the handoff's implied `0..40` | 20 is the wake value Ergomatic's SPM control has used since before this redesign (the now-deleted `SpmInput.tsx`'s own free-text-plus-steppers control, itself mirroring James's own rule for the wake value) — the redesign changed which control renders SPM, not what pressing `+` from empty should land on, so the value carried through unchanged. The handoff's `18`/`40` are its own prototype's picks, never reconciled against a real cadence; `10..60` matches `domain/validate.ts`'s actual `int(s.spm, 10, 60)` bound, so every value the stepper can produce is one the server actually accepts |
