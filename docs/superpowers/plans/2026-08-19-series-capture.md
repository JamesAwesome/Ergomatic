...
- Rests produce ZERO samples (the wire's clock freezes — proven); the series is a work-time trace, C2's own semantics. **[2026-08-20 correction, trace-truth Task 2: this held only for the MID-workout rest this plan's own Task 1 tested (step-3's 30s rest, a rest with a following interval to reset into) — not a general property. A rest whose wire keeps advancing DOES produce samples (21 on session-2-wu-4unequal.jsonl; 3 even on step-3's own trailing tail). `Sample.r` now marks them. Pinned in CI: `app/src/monitor/seriesRecorder.test.ts`'s "marks every sample recorded while the machine was resting (real capture, non-frozen rest)" — 21 rest samples against `docs/monitor/sessions/walk-2026-08-16/session-2-wu-4unequal.jsonl`, not a report only this session can reach.]**
