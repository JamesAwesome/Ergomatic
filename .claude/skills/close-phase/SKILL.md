...
`.claude/agent-briefing.md` — the file this skill tells its own subagents to
...
**Grouping.** One PR for all non-triad rows (CLAUDE.md, group-the-work). A
...
**Per row:** an implementer subagent (reads `.claude/agent-briefing.md`
...
9. **Teardown.** `git status` on the MAIN checkout first (stray writes there
   are a repeat failure, CLAUDE.md RF20, and are only cheap to fix while the
   branch exists), then
