...
- [ ] **Step 2: Prove the census can go red (RF21).** The whole point of this script over 1.75a's is the comment-leader strip, so that is what gets probed:
  ...
  - Delete the `sed -E` leader-strip from `norm()` and re-run **against `/tmp/pr175b-base`**: that hit disappears while the file still contains the phrase. **That is the red proof.** Restore, record both outputs.
