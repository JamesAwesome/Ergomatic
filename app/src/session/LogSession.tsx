      ...
      return submit(
        {
          ...
          // James's PR #230 review, P1b (Important) — RULED: no Save-time
          // reconciliation. A Save posts exactly `monitorRun`'s own
          // mount-time snapshot (tier B, the #228-accepted residual) —
          // never a number the screen didn't show (Gate 0). A machine
          // summary accepted LATE (during `useMonitorSession.ts`'s own
          // post-release teardown linger, after this screen already
          // mounted) is handled entirely on the producer side instead: it
          // RESTASHES the updated run into the slot (`reason=late-burst`,
          // that hook's own `summary-observations` handler) for the NEXT
          // `?from=monitor` arrival to serve, rather than reaching back
          // into an already-committed render. RC-3 (storage-spine design
          // spec §2, PR 1 Task 7): spread straight from `monitorRun`'s own
          // `summaryTotals`/`summaryDetail`/`verificationBytes`, never
          // re-derived.
