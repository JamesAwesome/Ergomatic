...
Three consequences run through the whole plan. **The producer order is 1) the rower's latest declaration, 2) our derivation from the profile, 3) refuse** — with a plausibility band on the raw profile number, so four of the SIX wrong-unit readings refuse loudly instead of silently classifying (observation 24, corrected 2026-09-03: the list is six readings, not five — the band admits hundredths-of-a-kg, which is the assumed-correct one, and hundredths-of-a-lb, which no band can exclude). **The SENT state names NEITHER** (James, 2026-09-04: "Stop talking about the weight class"): the rower sees the result id and nothing else, and the class plus its producer stay on the route's 200 and in the send's log line for an operator. The cost the ruling accepts: a DERIVED class is a guess about a fact Concept2 lets its owner set, and nobody now sees it as it is written. **Nothing is cached**: the class is read fresh on every send (ruling R13), because a declaration can change on Concept2 at any time with no signal to us.
...
| the plausibility band deleted | `Tests 2 failed | 37 passed` — "REFUSES four of the six wrong-unit readings" and "refuses an implausible weight WHATEVER the gender says" |
...
1. **The plausibility band cannot catch a hundredths-of-a-POUND reading, and the first draft of this plan claimed it caught all of them.** The band's own test went red on `16500`, which is inside 30-300 kg. A 2.2x unit error is undetectable from one number. The comment, the test and observation 24 now say "four of the SIX" (fix round 1, 2026-09-03: the readings list has six entries and the earlier "five" was an off-by-one repeated in five places), and the test pins BOTH admitted readings so a later overclaim goes red.
...
24. **The unit of `weight` on `GET /api/users/me` is UNMEASURED, and Concept2's own documentation contradicts itself about it.** The only primary line the vendor publishes sits on the **Create User** endpoint — a write parameter on the client-credentials admin route, not the read this PR consumes. Fetched 2026-09-03 from `https://log.concept2.com/developers/documentation/` (HTTP 200), the table row reads verbatim:
    ...
    **So the derivation carries a PLAUSIBILITY BAND on the raw number before it classifies** (`PLAUSIBLE_MIN/MAX_HUNDREDTHS_KG`, 3000..30000 = 30-300 kg), and a value outside it refuses with its own reason token rather than yielding a class. **Four of the six candidate readings then refuse loudly. Two do not, and the code and its test say so:** hundredths-of-a-kilogram is the one we ASSUME is right, so of course it passes the band; hundredths-of-a-pound differs from hundredths-of-a-kilogram by 2.2x, and no band wide enough to hold real rowers can exclude a 2.2x error. That residue is exactly what exit criterion 3b's TWO readings settle (Task 14), and it is bounded by the producer order: a rower who has declared a class on any recent Concept2 result never reaches this function at all.
...
| --- | --- | --- |
| `POST /api/logs` (`routes/data.ts`'s `checkCompletedAt` and `tzError`, and the `stores.logs.create` call beneath them) | `completedAt` (ISO 8601 string or null) · `tz` (canonical IANA zone or null) | 400 `field:"completedAt"` on a malformed stamp, and that is this route's ONLY refusal on the pair. **`tz` does not refuse at all: an unrecognised or empty zone is stored as `null` and the row SAVES** (Task 6 step 4b — a Concept2 field can never cost a rower their row; `tzError` reads the server image's own zone list, and the disagreeing list is ours). The strict 400 `field:"tz"` lives on the UPLOAD route (`routes/concept2.ts`), where a refusal costs one Concept2 send and leaves the rower's record intact. A PARSEABLE stamp outside the plausible band is likewise not a refusal: `checkCompletedAt` returns `{ok:true, value:null}` and the save survives with no stamp. |
...
- [ ] **Step 8: Commit, then probe.**
  ...
  **Re-measured 2026-09-03 against the blocks exactly as written above** (`mapping.test.ts` baseline `Tests 40 passed (40)`). The counts an earlier revision carried were measured against a 39-test file and against fixtures this fold changed; do not carry those forward. **Two of these did NOT bite as first written, and the fixtures above are the fix** — recorded here rather than quietly corrected, because a probe that stops biting when its fixture changes is RF21 arriving by drift:
  ...
  | M9f2 | `deriveWeightClass`: delete the plausibility band | RUN: `2 failed | 38 passed` — "REFUSES four of the six wrong-unit readings" and the refusal table's implausible case. Without the band an integer-kg reading classes every rower LIGHTWEIGHT, filing heavyweights in Concept2's lightweight rankings |
...
- [ ] **Step 8: Record what a walk must see.** This task changes a number on a permanent third-party record, so the design's exit evidence gains one observation, and it is not discharged by any gate in this repo (Task 14, and the walk card that carries exit criterion 2):
  ...
  > **The date Concept2 shows for an uploaded row is WITHIN SECONDS OF the moment the rower stopped rowing, not the moment they tapped Save.** The precondition that makes a NO possible: sit on the summary screen for a measurable interval — three minutes is plenty — before tapping Save, then read the C2 logbook entry's own date against the PM5's end-of-piece time. Without that deliberate gap the two clocks agree to within the noise and the observation proves nothing.

  **"Within seconds of", not "equals", and the difference is not hedging** (Task 6 fix round 1, F4). Concept2 asks for "the date as stored in the monitor"; we stamp `MonitorRun.completedAt` from the PHONE's clock at close-detection, which is a different clock reading a moment or two later. An "equals" pass condition could not be met by construction, and a walk observation nobody can answer YES to is decoration. Seconds of slop still fails loudly against a three-minute gap, which is the whole point of the precondition.

  Step 4b adds no walk observation of its own and should not: no gate in this repo can produce a phone whose tzdata disagrees with the server's, and the degrade is invisible when they agree. What it DOES owe the record is a line in the PR, and the FIRST version of that line was wrong (Task 6's report, deviation 2 — confirmed by the reviewer against the payload path). The accurate line: the route now stores `null` where it used to 400, and such a row still uploads with the workout's own end as its Concept2 date, labelled with the zone the phone reports at SEND time. It does not fall back to `loggedAt`: `routes/concept2.ts` builds its payload row as `{...eligibilityRow, tz: effectiveTz}`, never the raw `row.tz`, so `buildC2Payload`'s paired branch still fires on any row carrying a `completedAt`.
...
- [ ] **Step 2: The specific reconciliations.**
  ...
  12. **`POST /api/logs`'s tz refusal, wherever it is described as a refusal.** Task 6 step 4b makes an unrecognised zone a DEGRADE on that route while the upload route keeps its strict 400, and two committed tests said the opposite in their own titles. Reconcile every hit: the spec's own §Stored shapes line if it names the refusal, `server/db/schema.ts`'s `tz` comment (which item 9 above already rewrites), and the two test titles themselves. The invariant sentence — **a Concept2 field can never cost a rower their row** — goes in the spec beside the mapping row, not only in a code comment, because it is a product rule and the next person to tighten a validator needs to meet it.

      **The phrase list this item owes step 1 was wrong, and Task 6 fix round 1 corrected it.** Its three original phrases (`"tz must be an IANA timezone name"`, `"rejects an invalid tz"`, `"rejects a non-IANA tz"`) are all still in step 1's loop above, but the last two now return `(none)` — Task 6 renamed both test titles in the same commit that changed the behaviour, so those phrases can no longer find anything and are kept only as proof of that. **None of the three matched the place the contract was stated AS a contract**: this plan's own `POST /api/logs` API-table row, whose wording was "400 `field:\"tz\"` on anything not in `Intl.supportedValuesOf(\"timeZone\")` plus `\"UTC\"`". That row is corrected in the same round, and step 1's loop gains `"supportedValuesOf"` and `"field:\"tz\""` so a future sweep reaches any sibling stating it that way. Note that this plan is EXCLUDED from the census by its own `:!` pathspec, so a phrase alone would never have caught that row — a plan's own claims are reconciled by reading, which is why this note exists.

      **A second phrase in the same loop could not match either, and was shortened in the same round.** `"posted at save from PR2 on"` returned `(none)` — its target, `server/db/schema.ts`'s `tz` comment (item 9's whole subject), wraps mid-phrase across two comment lines, and `git grep -F` matches within a line. It is now `"posted at save from"`, which finds `schema.ts:381` plus the spec's §Stored-shapes sentence and the PR1 plan's copy of the same comment. **Run every census phrase once before trusting the loop:** a phrase that returns `(none)` because it cannot match reads exactly like a phrase that returns `(none)` because the work is done, and two of this list's entries were the first kind.
