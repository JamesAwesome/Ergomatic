...
| Measured row | **SUPERSEDED IN PART (James's tule-fog ruling, 2026-08-18 — see ROADMAP's queued target-judgment round): row judgments re-baseline from the session working average to EACH ROW'S OWN TARGET, with the target inline; the row semantics below describe what SHIPS TODAY and stand until that round lands.** README §8 geometry: index 14px · time 76px · pace 52px judged · deviation bar (14px track, center tick, 8px bar, width `min(50%, max(1.2%, |dev|/1.6 × 50%))`) · `+n.n`(slower)/`−n.n`(faster) vs the 2B working average. UNJUDGED measured row (the lone-measured-row gate, count < 2): NO bar, track, tick or ± — the same absence the Warm-up row renders, per 2B's absent-inputs rule (PM final-PR gate application, 2026-08-17: an abstention renders as absence, not as an empty widget) |
