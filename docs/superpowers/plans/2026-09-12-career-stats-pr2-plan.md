...
- [ ] **Step 3: Failing axis tests** (append to `axis.test.ts`; import `niceMax` and `domainFromReadings` from `./scale.js`): `formatTick` split `[115, 120, 125] → ["1:55", "2:00", "2:05"]`; metres `0 → "0"`, `5000 → "5,000"`, `20000 → "20,000"`; `niceMax(13000) → { max: 15000, step: 5000 }` (A3), `niceMax(43012) → { 60000, 20000 }`, `niceMax(0)` and `niceMax(1000) → { 1000, 1000 }`, `niceMax(80001) → { 100000, 50000 }`, `niceMax(2000000) → { 2000000, 500000 }`; **`niceMax(NaN)` and `niceMax(Infinity)` → `{ 1000, 1000 }`** — without the guard the ladder never exits, SYNCHRONOUSLY, so no vitest per-test `timeout` can interrupt it (measured: with `{ timeout: 2000 }` on the pin the whole run still hangs); the gate is an external `timeout` — run the mutation as `NODE_OPTIONS=--no-experimental-webstorage timeout 90 pnpm exec vitest run --project client src/charts/axis.test.ts; echo EXIT=$?` and read `EXIT=124` as the red; `chooseTicks([0, 15000], 4) → [0, 5000, 10000, 15000]`, `chooseTicks([0, 60000], 4) → [0, 20000, 40000, 60000]`, `chooseTicks([0, 1000], 2) → [0, 1000]`, `chooseTicks([0, 2000000], 5)` → five ticks by 500,000 — the rule `chooseTicks([0, max], max / step + 1)` reproduces `niceMax`'s grid; and the trend's domain from the primitives: `domainFromReadings([124.8, 117.6, 122.9, 115.3, 121.4, 114], { minHeight: 10 }) → [112, 126]` and `chooseTicks([112, 126], 4) → [115, 120, 125]` (all measured at `5c5099cb`).
...
export function niceMax(value: number, maxLines = 4): { max: number; step: number } {
  // NaN/Infinity never satisfy `max / step <= maxLines` and the ladder has no other exit: a non-finite value is the floor, not a hang.
...
- [ ] **Step 6: Gates.** `… --project unit domain/stats/calendar.test.ts` and `… --project client src/charts` → PASS (measured: charts 5 files / 34 tests incl. the existing). Mutations: `mondayOf` → `addDays(date, -dayOfWeek(date))` (Sunday start) → `× mondayOf: Sunday 2026-09-13 → 09-07 …`; drop the `Number.isFinite` guard → the run prints `RUN v4.1.11 …` and nothing more until `timeout` kills it: `EXIT=124` (both measured; RF40 — an exit ≥ 124 here is the finding, not a flake). `pnpm exec eslint domain/stats src/charts` clean.
...
export function metresPerWeek(rows: readonly DatedStatsRow[], range: DateRange, today: CalendarDate): WeekBar[] {
  // A CUSTOM `to` after today would anchor the window on a future week (this week mid-chart, five future baselines):
  const anchor = range.to === null || compareDates(range.to, today) > 0 ? today : range.to;
...
- [ ] **Step 4: Gates + mutations** (measured): client `src/you/stats` PASS. (a) render the line only when `rangeLine === ""` → `× the range line reads the days each preset covers …`. (b) `tabIndex={0}` on the chevron → PR 1's `× is exactly ONE focusable control …` AND the new chevron case. Contrast: chevron `--ink-3` on `--page` 6.69:1 (`contrast.json`). **The pressed fill is the house's FIRST `:active` rule and gets a browser gate in Task 9** (`design.spec.ts` `you screen`: `mouse.down` on the hero, computed `backgroundColor` equals `--surface-sunken` resolved in-page — `rgb(239, 234, 222)`; mutation: delete the rule → `Received: "rgba(0, 0, 0, 0)"`, measured). WKWebView tap behaviour for `:active` is UNTESTED until a device look — no `touchstart` listener exists in `src/`; a Record note, not a claim.
...
//   each pt → <circle className={`stats-dot stats-dot${cls}`} cx cy r={4}/>; last → <text className="stats-point-label" x={fits ? lx + 8 : lx - 8} textAnchor={fits ? "start" : "end"} y={y(…)} dominantBaseline="middle">{k.toUpperCase()} {fmtSplit(last.splitSeconds)}</text>   (lx = x(toDayNumber(last.date)), fits = lx + LABEL_ROOM <= W; cls = k === "6k" ? "-6k" : "")
```
...
- [ ] **Step 3: Gates + mutations** (measured): `src/you/stats` 40/40; the scan 4/4. (a) `const cls = "";` → the trend case (`.stats-dot-6k` 0). (b) `invert: false` → the trend case (`cy` 2k > 6k). (c) Task 4's date-ignored mutation → the trend case (`NOV` absent). (d) `textAnchor="start"` unconditionally (the flip removed) → `× a test on its month's last day flips its label …` (`expected 'start' to be 'end'`) (measured). Then `pnpm exec eslint src/you/stats src/api src/charts`, `pnpm exec tsc -p tsconfig.app.json --noEmit`, `pnpm exec prettier --check src domain` clean.
...
export async function seedGate0Tests(page: Page, ids: Record<string, string>, instantFor: (date: string) => string): Promise<Record<string, string>> {
  ...
  for (const t of GATE0_TEST_BODIES) {
    // A mistyped seed `log` must fail loudly, never become a silent throwaway (harden lens 2).
    if (t.log !== null && ids[t.log] === undefined) throw new Error(`${t.id}: no seeded log ${t.log}`);
...
13. §8.5 said the e2e asserts `7 OF 9` after the delete; ruling 19 struck that line and PR 1's e2e already did not. RESOLVED in this PR (`74415f88`) at the PM close gate's condition — the spec no longer says it.
...
16. **For James's hand-back, a ruling question and not a defect:** at 0 rows the whole page including TEST TREND is hidden (ruling 16), so a rower whose ONLY log is deleted keeps the test point (ruling 4) but never sees it until the next saved row.
17. The trend's `PAD_R 52` cannot hold a last-day-of-month label; the flip in Task 8 is the plan's, not the board's (which never drew such a test).
...
- `vitest --project unit domain/stats` → 7 files / 59 tests; `--project client src/you/stats src/api src/charts` → 22 files / 198 tests (+ `src/You.test.tsx`, 14, green); the lens-2 folds at `dd34e025`: tsc app + e2e 0, eslint 0 errors, the F2 clamp mutant `expected { y: 2026, m: 10, d: 12 } to strictly equal { y: 2026, m: 9, d: 7 }`, the F3 flip mutant `expected 'start' to be 'end'`, the F5 CSS mutant `Received: "rgba(0, 0, 0, 0)"` under `design.spec.ts -g "surface-sunken while pressed"`, F1 as recorded in Task 1. whole `pnpm test --project unit --project client` → 314 files, 8300 passed, 1 skipped.
- Mutations run and restored, each red on the named test: domain m1 Sunday start (7 failed — the received series `[2000, 0, 10000, 0, 0, 8000, 8000, 2000]`), m2 transposition (4), m3 fallback (2), m4a/m4b exclusive ends (4 each), m5 divisor (3), the ruling-22 keys-from-every-row (1), the key clamp against the straddle pin (1), m6 `<=` out-of-range (2), m7 `earliestDate` (1), m8 anchor-keyed current (1); client c1 range line (2), c2 every bar labelled (1), the F1 NaN guard (the whole vitest run hangs; `timeout 90` → `EXIT=124`), the F2 anchor clamp (1), the F3 label flip (1), the F5 `:active` rule deleted (design e2e, `rgba(0, 0, 0, 0)`), the raw-values aria-label (1: `Received: "Metres per week, eight weeks: 0, 0, 0, 0, 0, 13,000, 3,000, 2,000"`), the dropped all-zero branch (1), c3 filtered season (1), c4 chevron focusable (2), c6 6k class (1), c7 faster is down (1), c8 one-row season copy (1), the adapter's date-ignored (2); e2e: no backdate → red at the T1 read-back. Not run: PR 1's getters→UTC against the new tz file (same function, same shape), the two CSS mutations named in `design.spec.ts` — run at implementation.
