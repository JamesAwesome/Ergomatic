...
test("the page behind an open sheet does not scroll", async ({ page }) => {
  ...
  // this asserts it on Library because Library is the one surface whose
  // seeded list is reliably taller than the viewport. Today's fixture and a
  // two-interval log both measure ZERO scroll room, and a version of this
  // leg on Today passed 15/15 against a mutant with the lock deleted.
  //
  // In a BROWSER because the unit tests can only check that an inline style
  // was written; nothing in them goes red if `overflow: hidden` stops
  // preventing scroll. An antagonist pass measured the same behaviour on an
  // iOS 26.5 simulator with injected touch drags (unlocked 0 -> 1576, locked
  // 0 -> 0), so Chromium is a faithful stand-in here.
  await page.setViewportSize({ width: 390, height: 844 });
  await signInViaBackdoor(page, { email: "library-lock@e2e.test", name: "L" });
  await page.goto("/library");
  ...
  expect(
    room,
    "the page must be scrollable for this to mean anything",
  ).toBeGreaterThan(200);
