...
- **The response-site census is a COMMAND, not a count:** `grep -nE 'res\.(status\([0-9]+\)\.)?json\(' server/routes/data.ts` filtered to workout/log rows. At the plan's base that is: workouts list (row map), create (201), get, PUT, bulk `created` array; logs list, log get, PATCH no-op read (the empty-patch early return) AND PATCH update — NINE sites (workouts list, create, get, PUT, bulk `created`; logs list, get, PATCH no-op read, PATCH update — the earlier "ten" double-counted the log GET). The dual-field test drives every one.
