...
- [ ] `/session/run` (currently 6A's placeholder) becomes the real timer, reading the session draft — never the library workout — the same way Confirm targets already does

**Exit:** A confirmed session runs start to finish with the phone locked and unlocked mid-workout, with no lost state; a distance phase resolves a real average split with no PM5.
