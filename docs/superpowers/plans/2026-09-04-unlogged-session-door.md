...
- Update existing caller tests (`Today.test.tsx`, `LogSession.test.tsx`, `ConnectAction.test.tsx`, `WorkoutDetail.test.tsx`, `JustRow.test.tsx`, `AppRoutes.test.tsx`) and the new mounted `ReviewSession.test.tsx` harness. Recovery-specific coverage of the shared summary bodies and start hook may live in that mounted harness or their caller tests; edits to `PostWorkoutSummary.test.tsx`, `JustRowLog.test.tsx`, and `useStartWorkout.test.tsx` are not required solely for file-list compliance. Their unchanged legacy suites still run.
