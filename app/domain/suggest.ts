...
function byLeastRecentlyDone(a: LibraryEntry, b: LibraryEntry): number {
  if (a.lastDoneDaysAgo === null || b.lastDoneDaysAgo === null) {
    return (a.lastDoneDaysAgo === null ? 0 : 1) - (b.lastDoneDaysAgo === null ? 0 : 1)
  }
...
export function suggest(input: SuggestInput): Suggestion {
  ...
  const filtered = typeMatched.filter(
    (e) => prefs.difficulties.includes(e.difficulty) && e.estMinutes <= prefs.timeCapMinutes,
  )
  ...
  if (sorted.length === 0) {
    return { recommendationId: null, reason: `No ${matchType} sessions in your library.`, poolIds: [], fellBack: false }
  }

  const pickOverride = todayPickId ? sorted.find((e) => e.id === todayPickId) : undefined
