...
**Architecture:** Two small adapter-layer additions following the house native-first idiom (`isNative()` picks the arm; the native arm reaches its Capacitor plugin only through a dynamic `import()`). **Narrowed, fix round 2 (P2ii — the original wording overclaimed):** `@capacitor/browser` being absent from a flag-off `dist/client` is because the flag-off build has no reachable consumer, not because the dynamic import folds it out on its own. **Corrected 2026-09-01 (fix round 16): "nothing in `src/` consumes these modules" is now stale** — fix round 2's own dev-only probe (`src/monitor/Concept2LinkProbe.tsx`) imports `openExternalUrl` directly, behind `VITE_ENABLE_C2_LINK_PROBE`/`DEV`. Once compiled in, that `import()` emits its own lazy chunk that IS present in `dist/client`, simply never LOADED on web since `isNative()` is `false` there (`adapters/appLifecycle.ts` / `keepAwake.ts` are the precedents for the dispatch shape, not for a bundle-absence claim). Everything ships dark: nothing renders by default until PR2 (the probe card is itself flag-folded), and the server refuses while `C2_LINK_ENABLED` is off.
...
### Task 2: the return-to-app refresh seam (was "the foreground re-fetch seam")
...
- [ ] PR body: house shape, ~120 words, Record block carries the mutation ledger, the antagonist verdict (fix round 2, P2i: full TRIAD pass, not a skip — AUTH is in scope), the plan corrections, and the gate package pointer. James reviews; the design-gate ruling is RULED (2026-09-01, accept + gate (g) before flag flip — ROADMAP register / gate doc §6). No merge without his word.
