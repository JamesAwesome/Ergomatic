...
**Architecture:** Branch A (PROVEN at PR0: `state` echoes durably). The server holds `client_secret` and tokens; the browser hop carries no credential — a single-use, 15-minute `concept2_auth_attempts` nonce IS the user binding. Every route re-checks availability server-side (capability gate, not a hide). Upload reads only validated stored values, band-checks the two `machineSummary` fields, writes `c2_result_id`/`c2_user_id` only after C2's 2xx. **No automatic path ever deletes a link** — token-endpoint failures set `needs_reauth_at`, and refresh is serialized per user with `SELECT … FOR UPDATE`.
...
```ts
    ...
    // c2ResultId: C2's own result id, written ONLY after C2's 2xx (a 409
    // leaves it null). c2UserId: WHICH Concept2 account accepted it —
