...
| Flush | The recorder OWNS its flushes (antagonist B5: `recordActual` cannot see the buffer and returns early without saving on refused actuals — "riding" it was false): the hook layer flushes after each boundary write lands (~12 extra saves/session, counted honestly), on its own 30-second timer, and at close (after `completeMonitorRun`'s write). The recorder STOPS at close — a finish-grace actual after close updates the run per the existing rule but the series ends at close, stated |
