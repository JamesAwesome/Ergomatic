...
- [x] **GATED 2026-09-13 (`03878a0f`) — "a failing reconciliation does not
      ...
      **Preserved from the Icebox twin removed with this PR (James,
      2026-09-13), because this row never carried it:** the catch
      demonstrably WORKED all along — a forced `throw` placed inside the
      route's own try returns 200 and only the row assertion fails. This was
      a missing GATE, never a missing fix.
...
- [x] **CAPTURED 2026-09-14 (`2c9d7e09`) — the free-row summary's machine
      ...
      `terminate-sent` (the fake's terminate auto-cycle drains one status
      per 100 ms tick) and CLOSES ~1800 ms after the ended hand-off opens —
      `BURST_LINGER_MS` is 2000 ms and `summary-recorded` lands 200 ms after
      the 0x0039, on the hash sub-window. The capture delivers at +400 ms,
      ~310 ms past the opening bound and ~1.4 s inside the closing one.
      **`waitForTimeout` and not an observable, deliberately:** the DOM has
      ...
      not justify — so the dwell is a MEASURED number with margin at both
      ends, not a fourth guess. Gated by locators, never by the PNG (RF21):
      the tier block, AVG WATTS 300 / RATE 24 / DRAG 128, three dashes, and
      the pre-navigation URL check. Run RED first with the assertion and no
      delivery; the biting mutation is delivering at +0 ms.
...
- **Phase MD** — the monitor cluster's shallow seams got deep: one writer for
  the stored run, a lifecycle seam on the hook with the axes published once, one
  `Sample` declaration instead of six, and two explorations answered in writing
  (no freeze observer, no replay harness) · closed 2026-09-12 · #408, #413,
  #412, #414 · tester-invisible, no tag · receipts at close: `vi.doMock(` under
  `app/src/monitor/` 82 → 30 lines, `appLifecycle` mocks 26 → 0 statements;
  `saveMonitorRun` zero code references with `handoffStoreBoundary.test.ts`
  27 → 28 `it(`; the `r\??: true` grep prints exactly the three named lines,
  and the guard is the mapped type + `Pick` + witness, not the grep ·
  [detail](docs/history/phase-md.md)
