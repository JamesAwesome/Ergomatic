...
| Decision | Value |
|---|---|
| The split | `LogStep.actualSpm?: number` (new, additive, inside the existing `steps` jsonb — no SQL migration): the measured interval average, monitor door only. `spm` reverts to ONE meaning: the authored target rate, all doors. `buildMonitorLogSteps` moves its measured write to `actualSpm` and copies the authored rate from **`ProgramInterval.displaySpm`** (antagonist B2: the seed's steps are `{label, kind}` only). **AMENDED at Task 1 review (the discriminant was unsound for new dropped-measurement rows — a matched actual whose avgSpm is dropped produced a shape byte-identical to a pre-split row, and the renderer would print the TARGET as measured, the wrong-number class this phase exists to kill): on a MATCHED actual, the monitor builder writes `spm` ONLY alongside `actualSpm` — a dropped measurement writes NEITHER field, so new code can never reproduce the pre-split shape and the row-local discriminant is sound by construction. An UNMATCHED interval (no `actualSource`) still carries its authored `spm` — unambiguous, since the discriminant requires `actualSource === "pm5"`. Cost, accepted: a new matched-but-dropped row shows no SPM cell at all (absence over invention).** |
