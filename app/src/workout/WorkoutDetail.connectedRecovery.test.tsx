...
// `LogSession` mount that must find the machine's own data.
//
// RETARGETED (plan Task 5, hand-off store design spec): this file
// originally described a one-shot module SLOT
// (`stashHandoffRun`/`peekHandoffRun`) that the approved store protocol
// deletes outright (spec §2: "module slot + five stash sites -- deleted;
// the memory tier is current by construction"). There is no separate
// stash step any more: the record has lived in `handoffStore`'s own
// memory tier since the FIRST commit `createMonitorRun`'s call site made
// (accepted regardless of the durable write's own fate), so
// `LogSession.tsx`'s reader finds it there directly. Every comment below
// that used to say "the slot" now says "the store's memory tier."
//
...
describe("WorkoutDetail -> real connected recovery -> LogSession (James's PR #230 review, P2a)", () => {
  it("a held-error 'Log it anyway' press navigates through the REAL handleConnectedEnded into a Log screen served from the store's memory tier that POSTs the connected session", async () => {
    ...
    // must come from the STORE's own memory tier, never durable storage,
    // or this test would not be able to tell "the composition works" from
    // "storage happened to still hold something."
    ...
    // THE BUTTON PRESS THIS TEST EXISTS FOR: `proceedHandoff()` releases
    // the hold with no stash of its own to perform — the record (machine
    // numbers included) has lived in the store's memory tier since the
    // create-commit, unaffected by the durable write's own denial — which
    // is what fires `onEnded` -> the REAL `handleConnectedEnded` ->
    // `navigate("/library/:id/log?from=monitor")`.
    await userEvent.click(
      screen.getByRole("button", { name: "Log it anyway" }),
    );

    // The REAL LogSession, reached through the REAL navigation, serving
    // the record from the store's memory tier (durable storage is still
    // empty — asserted again below).
    ...
    // store's memory tier -> the POST — present only on the connected
    // path (the manual door never sets this field at all) — and the real
    // machine-summary totals `deliverSummary` put on the wire, folded in
    // memory because durable storage never held them.
