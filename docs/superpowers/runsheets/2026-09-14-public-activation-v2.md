# Runsheet — public activation and the stranger sign-up, v2

**Status: DRAFT, awaiting the PM readiness gate. v1 was NOT READY on twelve
counts; this is the fold. Do not run this yet.**
...
- **Host:** main at **`1aebec39`**, seven commits ahead of that tag, including
  ...
  actually running it** — the denial and confirmation strings below are the
  host's, not the phone bundle's, wherever they are served by the API.
...
| **DOOR OPEN — the safety clock** | **20 min HARD** | step 2's recreate returns | step 9's recreate returns |
| Stage 2 cases | 12 min of that 20 | A1b | A5 |
| Step 9, closing | **outside every cap** | always runs | always runs |
...
| S3 | Complete it | `<address> isn't invited to this Ergomatic. Ask the owner to add you.` | **This is the control.** PASS: the denial. FAIL: anything else, and STOP — the gate is not behaving as `restricted` before you open it |
...
|---|---|---|---|
| A1b | Sign in with the SAME second account from S3 | the first-account confirmation, naming it | **PASS: the same identity that was denied at S3 is now admitted.** That contrast is the session's result; the screen alone is not. FAIL: the denial again |
...
| A4 | You → **Delete account**, complete the re-auth | signed out | **This is TEARDOWN, not a test.** An account created under public mode and left alive becomes the locked-out-rower exposure the moment step 9 restores `restricted`. Its evidence value is a bonus |
...
2. **Open the door.** Editor-free, and the backup lives OUTSIDE the checkout so
   ...
   **What a typo looks like, so you do not diagnose it live:** any value that is
   ...
   fails after 120 s, and the site is down. That is a step 9, immediately.
3. **Start the 20-minute alarm.** Run stage 2.
4. **STOP at 20 minutes** wherever you are.
9. **Close the door**, by restore rather than edit:
...
10. **Sign back into your own account on the phone.**
...
**On any abort, go to step 9.** It is outside every clock and it is the same
pair of commands you already ran once.

**And the honest part v1 got wrong: step 9 is not guaranteed to run.** If SSH
...
run step 9; if you cannot, the safer resting state is the site DOWN rather than
the door open. The only detector we have is step 1's count against step 9's.
...
The only durable artefacts are step 1's and step 9's count outputs and one
