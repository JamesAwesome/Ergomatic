...
| `GET /api/concept2/link` | user | `200 {available:false}` \| `{available:true,linked:false}` \| `{available:true,linked:true,weightClass,needsReauth}` | never 4xx/5xx for availability; never tokens |
...
- Route logic locked:
  ...
  - **Link GET** (requireUser): unavailable → `200 {available:false}` (200 on purpose — the matrix's one non-403 row). Else `{available:true, linked, weightClass?, needsReauth: link.needsReauthAt !== null}`. Tokens never serialized.
