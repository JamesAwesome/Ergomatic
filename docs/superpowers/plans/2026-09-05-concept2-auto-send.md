...
**Tech Stack:** Express 5 + Drizzle/Postgres (migration 0025), React 19, Vitest (unit/client/integration), Playwright.
...
**Files:** `app/server/db/schema.ts`, `app/drizzle/0025_classy_red_ghost.sql` + `app/drizzle/meta/{_journal.json,0025_snapshot.json}` (generated by `pnpm db:generate`, header added by hand), `app/server/stores/concept2.ts`, `app/server/testing/fakes.ts`, `app/server/stores/concept2.integration.test.ts`.
...
**Renumbered at the merge of main (PM gate C1):** the branch minted this migration as `0024` off `85a0de9c`; Phase DE PR 2 (#310) landed `0024_pain_to_effort` on main the same day, so the merge took main's 0024, deleted ours and regenerated the identical three statements as `0025_classy_red_ghost` (`pnpm db:generate`) — the case spec §3.1's numbering bullet named. The diff below still says 0024; the head says 0025.
...
```
9a73a8a9 Auto-send: auto_send and send_failed_* on concept2_links (store, migration 0025)

 app/drizzle/0025_classy_red_ghost.sql      |   20 +
 app/drizzle/meta/0025_snapshot.json            | 1162 ++++++++++++++++++++++++
...
```diff
diff --git a/app/drizzle/0025_classy_red_ghost.sql b/app/drizzle/0025_classy_red_ghost.sql
new file mode 100644
index 00000000..453d389f
--- /dev/null
+++ b/app/drizzle/0025_classy_red_ghost.sql
@@ -0,0 +1,20 @@
...
diff --git a/app/drizzle/meta/0025_snapshot.json b/app/drizzle/meta/0025_snapshot.json
new file mode 100644
index 00000000..8269abdd
--- /dev/null
+++ b/app/drizzle/meta/0025_snapshot.json
@@ -0,0 +1,1162 @@
+{
...
**Files:** `docs/superpowers/specs/2026-09-05-concept2-auto-send-design.md` (rev 4: the two as-built mechanisms in §3.3), `docs/design/handoffs/2026-08-31-concept2-connect/amendment-2026-09-05-autosend.html` (§9 Built — the landscape mode-line departure, the ink remedy, segment padding), `docs/design/handoffs/2026-08-31-concept2-connect/README.md` (intro; 1c/1d superseded notes; the linked-state line), `ROADMAP.md` (Gate 0 approved + BUILT status on the auto-send row; ticks at merge), `docs/RELEASING.md` ("Not a floor — migration 0025": defaulted/nullable columns; PATCH 404s on an older server and the card says so).
